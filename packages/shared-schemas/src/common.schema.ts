...
export const DecimalStringSchema = z.string().regex(/^\d+(\.\d+)?$/, {
  error: 'Must be a decimal string (e.g. "1.50", "0.72")',
});

/** Price in [0, 1] exclusive — Polymarket outcome price format */
export const PriceStringSchema = z.string().regex(/^0\.\d+$/, {
  error: 'Price must be a decimal between 0 and 1 exclusive (e.g. "0.72")',
});
...
export const IsoDateSchema = z
  .string()
  .datetime({ error: "Must be an ISO 8601 datetime string" });
