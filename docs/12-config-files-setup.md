...
> The full schema content is in `12-config-files-setup.md` (previous version). Copy the `schema.prisma` content from there — it is unchanged except that `UserCredential`, `AuditLog`, `Admin`, and `AdminSession` models have been removed (they now live in `schema.admin.prisma`).
...
*Reference: [Dev Setup Guide](./09-dev-setup.md) · [Database & Redis](./04-database-and-redis.md) · [Env Reference](./10-env-reference.md)*
