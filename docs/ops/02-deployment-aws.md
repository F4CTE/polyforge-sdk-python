...
> Pre-deployment audit completed 2026-03-28. All critical, high, and medium issues resolved. See `docs/reports/AUDIT-2026-03-28.md` for the full audit report.
...
aws secretsmanager create-secret --name polyforge/API_SERVICE_INTERNAL_JWT_AUDIENCE         --secret-string "api-service"
aws secretsmanager create-secret --name polyforge/API_SERVICE_INTERNAL_JWT_ISSUERS          --secret-string "auth-service,admin-api-service,bot-service,market-data-service,order-service"
aws secretsmanager create-secret --name polyforge/AUTH_SERVICE_INTERNAL_JWT_AUDIENCE        --secret-string "auth-service"
aws secretsmanager create-secret --name polyforge/AUTH_SERVICE_INTERNAL_JWT_ISSUERS         --secret-string "api-service,admin-api-service,bot-service,market-data-service,order-service"
aws secretsmanager create-secret --name polyforge/MARKET_DATA_SERVICE_INTERNAL_JWT_AUDIENCE --secret-string "market-data-service"
aws secretsmanager create-secret --name polyforge/MARKET_DATA_SERVICE_INTERNAL_JWT_ISSUERS  --secret-string "api-service,auth-service,admin-api-service,bot-service,order-service"
...
# After Polymarket Builder Program approval (see 07-polymarket-integration.md):
...
> - All other services use `node:24-alpine`
...
jobs:
  ...
  deploy:
    ...
    steps:
      ...
      # NOTE: signer-service and strategy-engine Dockerfiles include Rust build stages
      # (rust:1-slim). Docker handles the Rust compilation internally — no Rust needed
      # on the CI runner itself. Build times ~5min for these two services.
...
Security
  ...
  ☐ signer-service logs: "Rust NAPI-RS encryption active — memory-safe key handling enabled"
  ☐ strategy-engine starts without WASM errors (sandboxed evaluation active)
  ☐ All services have no-new-privileges + cap_drop ALL (verify: docker inspect)
  ☐ No dev- or CHANGE_ME secrets (startup validation rejects them)
```
...
*Previous: [Testing & Practices](../05-testing-and-practices.md) | Next: [Polymarket Integration](../07-polymarket-integration.md)*
