...
| Requirement | Details |
|---|---|
| AWS account | With IAM admin access (or the specific policies listed in [02-deployment-aws.md](./02-deployment-aws.md) Part 1) |
...
| Polymarket Builder API keys | From the Builder Program dashboard (see [07-polymarket-integration.md](../07-polymarket-integration.md)) |
...
If any of these are missing, install manually following the commands in [02-deployment-aws.md](./02-deployment-aws.md) Part 6.4.
...
- **signer-service:** Compiles a NAPI-RS native addon (`rust:1-slim`) for memory-safe key encryption. Runtime uses `node:26-slim` (Debian, not Alpine) for glibc compatibility.
...
See the detailed post-deployment checklist in [02-deployment-aws.md](./02-deployment-aws.md) (Post-Deployment Checklist section) covering SSL, email, API, Polymarket integration, bots, monitoring, and security checks.
...
See [04-backup-recovery.md](./04-backup-recovery.md) for full database recovery procedures.
...
Or let GitHub Actions handle it automatically on push to `main` (see [02-deployment-aws.md](./02-deployment-aws.md) Part 12).
...
See [04-backup-recovery.md](./04-backup-recovery.md) for the full backup strategy and recovery procedures.
...
*Related docs: [Architecture](../01-architecture.md) | [Deployment (AWS manual)](./02-deployment-aws.md) | [Launch Runbook](./03-launch-runbook.md) | [Backup & Recovery](./04-backup-recovery.md) | [Incident Response](./05-incident-response.md) | [Performance Tuning](./06-performance-tuning.md)*
