function Footer() {
  const cols = [
    { title: 'Product', links: ['Strategy builder', 'Whale intelligence', 'Backtest', 'Paper trading', 'Marketplace'] },
    { title: 'Developers', links: ['API reference', 'TypeScript SDK', 'Python SDK', 'Rust SDK', 'MCP server'] },
    { title: 'Company', links: ['About', 'Changelog', 'Roadmap', 'Status', 'Contact'] },
    ...
    <footer className="pf-footer">
      <div className="container">
        <div className="footer-grid">
          <div>
            <a href="#top" style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-primary)', textDecoration: 'none', fontWeight: 600, fontSize: 16 }}>
          ...
          {cols.map(c => (
            <div key={c.title}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {c.links.map(l => (
                  <li key={l}><a href="#" style={{ color: 'var(--text-secondary)', fontSize: 13, textDecoration: 'none', transition: 'color 120ms' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--text-secondary)'}>{l}</a></li>
                ))}
              </ul>
            </div>
          ))}
