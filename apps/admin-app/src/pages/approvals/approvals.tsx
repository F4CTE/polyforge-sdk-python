    ...
    <div className="animate-fade-in p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-elevated border border-default rounded-pf-lg overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-tertiary mt-1">All users have been reviewed.</p>
          </div>
        ) : (
              ...
              {users.map(u => (
                <tr key={u.id} className="hover:bg-surface/50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-primary">{u.username}</div>
                    <div className="text-xs text-tertiary font-mono">{u.id.slice(0, 8)}…</div>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-4">
          ...
          <span className="text-sm font-mono text-secondary">{page} / {totalPages}</span>
