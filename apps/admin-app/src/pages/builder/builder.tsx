...
export function Component() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64" role="status" aria-label="Loading builder statistics">
        <div className="text-sm text-pf-text-secondary">Loading builder stats...</div>
      </div>
    );
  }
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-pf-text-tertiary">No builder data available</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <h2 className="text-lg font-semibold text-pf-text">Builder Program</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-pf-elevated border border-pf-border rounded-pf-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-pf-text-tertiary">Current Tier</span>
          </div>
          <div className="text-2xl font-bold text-pf-text capitalize">{stats.tier ?? 'N/A'}</div>
        </div>
        <div className="bg-pf-elevated border border-pf-border rounded-pf-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <DollarSign size={16} className="text-pf-success" aria-hidden="true" />
            <span className="text-xs text-pf-text-tertiary">Weekly Reward</span>
          </div>
          <div className="text-2xl font-bold text-pf-text">${stats.weeklyRewardUsdc ?? '0'}</div>
        </div>
        <div className="bg-pf-elevated border border-pf-border rounded-pf-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp size={16} className="text-pf-info" aria-hidden="true" />
            <span className="text-xs text-pf-text-tertiary">Attributed Volume</span>
          </div>
          <div className="text-2xl font-bold text-pf-text">${Number(stats.attributedVolume ?? 0).toLocaleString()}</div>
        </div>
      </div>
      ...
      <div className="bg-pf-elevated border border-pf-border rounded-pf-lg p-5">
        <div className="flex items-center gap-2 mb-4">
          <Hammer size={16} className="text-pf-cyan-500" aria-hidden="true" />
          <h3 className="text-sm font-semibold text-pf-text">Weekly History</h3>
        </div>
            ...
            <table className="w-full text-sm" aria-label="Strategy builder blocks">
              ...
              <thead>
                <tr className="border-b border-pf-border">
                  <th scope="col" className="text-left px-3 py-2 text-xs font-medium text-pf-text-tertiary uppercase">Week</th>
                  <th scope="col" className="text-right px-3 py-2 text-xs font-medium text-pf-text-tertiary uppercase">Volume</th>
                  <th scope="col" className="text-right px-3 py-2 text-xs font-medium text-pf-text-tertiary uppercase">Reward</th>
                </tr>
              </thead>
              <tbody>
                {stats.weekly.map((w) => (
