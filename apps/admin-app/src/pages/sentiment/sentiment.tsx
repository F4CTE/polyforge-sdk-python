...
function SortIcon({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey; sortDir: SortDir }) {
  ...
  return sortDir === 'asc'
    ? <ArrowUp size={12} className="text-accent ml-1 inline-block" aria-hidden="true" />
    : <ArrowDown size={12} className="text-accent ml-1 inline-block" aria-hidden="true" />;
}
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-primary">Market Sentiment</h2>
        <Button
          type="button"
          variant="ghost"
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard
          ...
          colorClass={summary ? scoreColor(summary.avgScore) : 'text-primary'}
          loading={loadingSummary}
        />
      </div>
        ...
        {loadingCategory ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            ...
            >
              ...
              <Bar dataKey="neutralPct" name="Neutral %" stackId="a" fill="var(--text-tertiary)" opacity={0.45}>
                {chartData.map((entry) => (
                  <Cell key={entry.category} fill="var(--text-tertiary)" />
                ))}
              </Bar>
              <Bar dataKey="bearishPct" name="Bearish %" stackId="a" fill="var(--loss)" radius={[0, 4, 4, 0]}>
                {chartData.map((entry) => (
                  <Cell key={entry.category} fill="var(--loss)" />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
                ...
                processed.map((row) => {
                  ...
                  return (
                    <tr key={row.marketId} className="border-b border-default last:border-0 hover:bg-app transition-colors">
                      ...
                      <td className="px-4 py-3 text-right text-gain font-mono">{row.bullishCount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right text-loss font-mono">{row.bearishCount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right text-secondary font-mono">{row.signalCount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-tertiary">{formatDateTime(row.lastUpdated)}</td>
                    </tr>
                  );
                })
              )}
