    ...
    return (
      <span className="inline-flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        ...
        <Button
          ...
          onClick={save}
          disabled={saving}
          className="text-gain hover:text-gain/80 disabled:opacity-40 transition-colors"
...
function CorrelationBar({ value }: { value: number }) {
  ...
  const color =
    pct >= 70 ? 'bg-gain' : pct >= 40 ? 'bg-warning' : 'bg-loss';
  const textColor =
    pct >= 70 ? 'text-gain' : pct >= 40 ? 'text-warning' : 'text-loss';
  return (
    <span
        ...
        <tbody>
          {[0, 1].map((i) => (
            <tr key={i} className="border-b border-default/40 animate-shimmer">
              ...
              <td className="py-3 pr-4"><div className="h-4 w-14 bg-overlay rounded" /></td>
              <td className="py-3 pr-4"><div className="h-4 w-12 bg-overlay rounded" /></td>
              <td className="py-3"><div className="h-5 w-12 bg-overlay rounded" /></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function AnalyticsPanel({ analytics, loading, expanded, onToggle }: AnalyticsPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-default bg-elevated overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 font-medium text-primary mr-auto">
          <BarChart2 className="size-4 text-accent-text shrink-0" aria-hidden="true" />
          Copy Analytics
        </span>
        ...
        {!loading && analytics && (
          <>
            <span className="flex items-center gap-2 text-secondary">
              Total P&amp;L:{' '}
              <span className={`font-mono font-semibold ${pnlPositive ? 'text-gain' : 'text-loss'}`}>
                {analytics.totalCopiedPnl}
              </span>
            </span>
            <span className="text-strong hidden sm:inline">|</span>
            <span className="text-secondary">
              ...
              <span className="font-mono text-gain">{analytics.bestPerformer.pnl}</span>
            </span>
            <span className="text-strong hidden sm:inline">|</span>
            <span className="text-secondary">
              Worst:{' '}
              <Link
                ...
                onClick={(e) => e.stopPropagation()}
                className="text-loss hover:underline font-medium"
              >
                @{analytics.worstPerformer.username}
              </Link>{' '}
              <span className="font-mono text-loss">{analytics.worstPerformer.pnl}</span>
            </span>
            <span className="text-strong hidden sm:inline">|</span>
            <span className="text-secondary">
              Active:{' '}
              <span className="font-medium text-primary">{analytics.activeCopies}</span>
            </span>
          </>
        )}
      ...
      {/* Expanded section */}
      {expanded && (
        <div id="copy-analytics-expanded" className="border-t border-default px-4 pb-4">
                ...
                <tbody>
                  {analytics.traders.map((t) => {
                    ...
                    return (
                      <tr
                        key={t.username}
                        className="border-b border-default/40 last:border-0 hover:bg-surface/30 transition-colors"
                      >
                        {/* Trader */}
                        <td className="py-3 pr-4">
                          ...
                          >
                            ...
                            <span className="font-medium text-primary group-hover/trader:text-accent-text transition-colors">
                              @{t.username}
                            </span>
                          </Link>
                        </td>
                        ...
                        {/* Copied P&L */}
                        <td className="py-3 pr-4">
                          <span className={`font-mono font-semibold ${pnlUp ? 'text-gain' : 'text-loss'}`}>
                            {t.copiedPnl}
                          </span>
                        </td>
                        ...
                        {/* Max Drawdown */}
                        <td className="py-3 pr-4">
                          <span className="font-mono text-loss">{t.maxDrawdown}</span>
                        </td>

                        {/* Win Rate */}
                        <td className="py-3 pr-4">
                          <span className="font-mono text-secondary">{t.winRate.toFixed(1)}%</span>
                        </td>
    ...
    <div className="animate-fade-in p-6 max-w-7xl mx-auto space-y-6">
      ...
      {!loading && configs.length > 0 && (() => {
        ...
        return (
          <div className="flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl bg-elevated border border-default text-sm">
            ...
            <span className="text-secondary">
              Total copied P&L:{' '}
              <span className={`font-mono font-medium ${pnlPositive ? 'text-gain' : 'text-loss'}`}>
                {formatPnl(totalPnl)}
              </span>
            </span>
            <span className="text-strong">|</span>
            <span className="text-secondary">
              <span className="font-medium text-gain">{active} active</span>
              {paused > 0 && (
                <>, <span className="font-medium text-warning">{paused} paused</span></>
              )}
            </span>
          </div>
        );
      ...
      {!loading && configs.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <svg className="size-10 text-tertiary mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          {configs.map((config) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      className={`font-mono font-semibold ${
                        config.copiedPnl
                          ? config.copiedPnl.startsWith('-') ? 'text-loss' : 'text-gain'
                          : config.totalPnl >= 0 ? 'text-gain' : 'text-loss'
                      }`}
                    >
                ...
                {/* Action buttons */}
                <div
                  className="flex items-center justify-between gap-1 pt-3 border-t border-subtle"
                  onClick={(e) => e.stopPropagation()}
                >
