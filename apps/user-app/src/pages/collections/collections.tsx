    ...
    <div className="animate-fade-in p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-pf-text-muted mt-1">Curated strategy bundles from the PolyForge team</p>
        </div>
      </div>
          ...
          {collections.map(col => (
            ...
            >
              ...
              {col.coverListings.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {col.coverListings.slice(0, 3).map(listing => (
                    <span
                      key={listing.id}
                      className="px-2 py-1 rounded-pf-full text-pf-label bg-pf-overlay text-pf-text-secondary border border-pf-border truncate max-w-[140px]"
                      title={listing.title}
                    >
                      {listing.title}
                    </span>
              ...
              <Button
                ...
                className="mt-auto flex items-center justify-center gap-2 w-full py-2 rounded-pf-sm text-sm font-medium border border-pf-cyan-500/40 text-pf-cyan-400 hover:bg-pf-cyan-500/10 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-pf-cyan-400"
              >
                Browse Collection
