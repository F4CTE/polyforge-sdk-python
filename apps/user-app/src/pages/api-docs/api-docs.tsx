...
const METHOD_CLS: Record<string, string> = {
  ...
  POST:   'bg-sky-500/15 text-sky-400 ring-1 ring-sky-500/30',
  PATCH:  'bg-amber-500/15 text-amber-400 ring-1 ring-amber-500/30',
  DELETE: 'bg-red-500/15 text-red-400 ring-1 ring-red-500/30',
};
const METHOD_BORDER: Record<string, string> = {
  GET:    'border-l-emerald-500/70',
  POST:   'border-l-sky-500/70',
  PATCH:  'border-l-amber-500/70',
...
const LANG_COLOR: Record<Lang, string> = { curl: 'text-amber-400', ts: 'text-sky-400', py: 'text-emerald-400', rust: 'text-orange-400' };

/* ─── Primitive components ───────────────────────────────────────── */

function Badge({ text, cls }: { text: string; cls: string }) {
  return (
    <span className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md tracking-wide ${cls}`}>
      {text}
    </span>
  );
}
...
function Code({ code, label }: { code: string; label?: string }) {
  return (
    <div className="rounded-lg overflow-hidden border border-pf-border">
      <div className="flex items-center gap-2 px-4 py-2 bg-pf-overlay/60 border-b border-pf-border">
        <div className="flex gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-pf-border-strong" />
          <span className="w-2.5 h-2.5 rounded-full bg-pf-border-strong" />
          <span className="w-2.5 h-2.5 rounded-full bg-pf-border-strong" />
        </div>
        {label && <span className={`text-[10px] font-mono ml-1 ${LANG_COLOR[label as Lang] ?? 'text-pf-text-muted'}`}>{label}</span>}
      </div>
      <pre className="bg-pf-base p-4 text-[11.5px] font-mono text-pf-text overflow-x-auto whitespace-pre leading-[1.6]">
        {code.trim()}
      </pre>
    </div>
  );
...
function InlineCode({ children }: { children: string }) {
  return (
    <code className="bg-pf-overlay/80 border border-pf-border-subtle px-1.5 py-0.5 rounded text-[11px] font-mono text-pf-cyan-300">
      {children}
    </code>
  );
}
...
function FieldTable({ fields }: { fields: EndpointField[] }) {
  return (
    <div className="rounded-lg border border-pf-border overflow-hidden">
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-pf-overlay/40 border-b border-pf-border">
            <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Field</th>
            <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Type</th>
            <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Req.</th>
            <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Description</th>
          </tr>
        </thead>
        <tbody>
          {fields.map((f, i) => (
            <tr key={f.name} className={i < fields.length - 1 ? 'border-b border-pf-border-subtle' : ''}>
              <td className="px-4 py-2.5 font-mono text-pf-cyan-300 text-[11px]">{f.name}</td>
              <td className="px-4 py-2.5 text-pf-text-secondary">{f.type}</td>
              <td className="px-4 py-2.5 text-pf-text-muted">{f.required ? <span className="text-pf-cyan-400">✓</span> : '—'}</td>
              <td className="px-4 py-2.5 text-pf-text-secondary leading-relaxed">{f.description}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function LangTabs({ lang, setLang, available }: { lang: Lang; setLang: (l: Lang) => void; available: Lang[] }) {
  return (
    <div className="flex gap-1 p-1 bg-pf-overlay/50 rounded-lg w-fit mb-3">
      {available.map(l => (
        <button
          key={l}
          onClick={() => setLang(l)}
          className={`px-3 py-1 text-[11px] font-medium rounded-md transition-all duration-150 ${
            lang === l
              ? 'bg-pf-surface text-pf-text shadow-sm ring-1 ring-pf-border'
...
function EndpointCard({ ep, lang, setLang }: { ep: EndpointDef; lang: Lang; setLang: (l: Lang) => void }) {
  ...
  return (
    <div className={`border border-pf-border border-l-[3px] ${METHOD_BORDER[ep.method]} rounded-lg overflow-hidden transition-all duration-150`}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-3 px-4 py-3.5 bg-pf-surface hover:bg-pf-elevated text-left transition-colors duration-150 cursor-pointer"
        aria-expanded={open}
      >
        <Badge text={ep.method} cls={METHOD_CLS[ep.method]} />
        <code className="flex-1 text-[12.5px] font-mono text-pf-text">{ep.path}</code>
        ...
        {open
          ? <ChevronDown className="w-4 h-4 text-pf-text-muted shrink-0" />
          : <ChevronRight className="w-4 h-4 text-pf-text-muted shrink-0" />
        }
      </button>

      {open && (
        <div className="border-t border-pf-border bg-pf-base/50 px-4 py-5 space-y-5">
          ...
          {ep.responseNote && (
            <p className="text-xs text-pf-text-muted bg-pf-overlay/40 rounded-lg px-3 py-2.5 border border-pf-border-subtle">
              {ep.responseNote}
            </p>
          )}
          {available.length > 0 && (
...
function PageTitle({ title, subtitle, count }: { title: string; subtitle?: string; count?: number }) {
  return (
    <div className="pb-6 mb-7 border-b border-pf-border-subtle">
      <div className="flex items-center gap-3 mb-2">
        <h1 className="text-[22px] font-bold text-pf-text tracking-tight">{title}</h1>
        {count !== undefined && (
          <span className="text-[11px] font-mono bg-pf-overlay/80 text-pf-text-muted px-2 py-0.5 rounded-md ring-1 ring-pf-border">
            {count} endpoint{count !== 1 ? 's' : ''}
          </span>
        )}
      </div>
      {subtitle && <p className="text-sm text-pf-text-secondary leading-relaxed max-w-2xl">{subtitle}</p>}
    </div>
  );
}
...
export function Component() {
  ...
  function renderContent() {
    ...
    if (activeId === 'getting-started') return (
      <div className="space-y-6">
        ...
        />
        {/* Base URL hero */}
        <div className="rounded-xl border border-pf-border bg-gradient-to-br from-pf-surface to-pf-base p-5 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-pf-text-muted mb-1">Base URL</p>
            <code className="text-base font-mono text-pf-cyan-300 tracking-tight">https://api.polyforge.app/api/v1</code>
          </div>
          <div className="flex gap-2">
            <span className="text-xs bg-pf-overlay/80 border border-pf-border text-pf-text-secondary px-3 py-1.5 rounded-lg">REST · JSON</span>
            <span className="text-xs bg-pf-overlay/80 border border-pf-border text-pf-text-secondary px-3 py-1.5 rounded-lg">TLS 1.2+</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={step} className="flex gap-4 p-5 bg-pf-surface border border-pf-border rounded-xl">
    ...
    if (activeId === 'authentication') return (
      <div className="space-y-6">
        <PageTitle
          ...
          subtitle="All requests use Bearer token authentication. API keys are recommended for external integrations — they are scoped, rotatable, and do not expire by default."
        />
        <Code code={'Authorization: Bearer pf_live_abc123...\nContent-Type: application/json'} label="curl" />
        ...
        <Sub title="Scopes">
          <div className="space-y-2">
            {([
              ...
              <div key={s.scope} className="flex items-start gap-3 p-3.5 bg-pf-surface border border-pf-border rounded-lg">
    ...
    if (activeId === 'sdks') return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {([
            { lang: 'TypeScript', install: 'npm install @polyforge/sdk', color: 'text-sky-400', border: 'border-sky-500/30', code: `import { PolyforgeClient } from '@polyforge/sdk';\nconst client = new PolyforgeClient({ apiKey: 'pf_...' });` },
            { lang: 'Python',     install: 'pip install polyforge',      color: 'text-emerald-400', border: 'border-emerald-500/30', code: `from polyforge import PolyforgeClient\nclient = PolyforgeClient(api_key='pf_...')` },
            { lang: 'Rust',       install: 'cargo add polyforge tokio',  color: 'text-orange-400', border: 'border-orange-500/30', code: `use polyforge::PolyforgeClient;\nlet client = PolyforgeClient::new("pf_...");` },
          ]).map(s => (
            <div key={s.lang} className={`bg-pf-surface border ${s.border} rounded-xl p-4 space-y-3`}>
        ...
        <div className="p-5 bg-pf-surface border border-pf-purple-500/30 rounded-xl space-y-3">
          <div className="flex items-center gap-2">
            <p className="text-sm font-bold text-pf-purple-300">MCP Server</p>
            <span className="text-[10px] bg-pf-purple-500/15 text-pf-purple-400 px-2 py-0.5 rounded-md ring-1 ring-pf-purple-500/30">Claude Desktop</span>
          </div>
          <Code code={'npx @polyforge/mcp-server'} label="curl" />
    ...
    if (activeId === 'websocket') return (
      <div className="space-y-6">
        ...
        <Sub title="Connection">
          <Code code={'wss://api.polyforge.app/ws?token=<JWT>'} label="curl" />
        </Sub>
        <Sub title="Subscribe to a strategy">
          <Code code={`// Send after connection opens\n{ "event": "subscribe", "data": { "strategyId": "strat-uuid" } }`} label="ts" />
        </Sub>
        <Sub title="Inbound event types">
          <div className="rounded-lg border border-pf-border overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-pf-overlay/40 border-b border-pf-border">
                  <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Event</th>
                  <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Payload</th>
                </tr>
              </thead>
              <tbody>
                {[
                ...
                ].map(([ev, payload], i, arr) => (
                  <tr key={ev} className={i < arr.length - 1 ? 'border-b border-pf-border-subtle' : ''}>
                    <td className="px-4 py-2.5 font-mono text-pf-cyan-300">{ev}</td>
                    <td className="px-4 py-2.5 text-pf-text-secondary font-mono">{payload}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    if (activeId === 'sse') return (
      <div className="space-y-6">
        <PageTitle
          title="SSE Events"
          subtitle="Streams execution events for a single strategy over a persistent HTTP connection using text/event-stream. Ideal for API-key-authenticated clients that cannot use WebSocket."
        />
        <Sub title="Endpoint">
          <Code code={'GET /api/v1/strategies/:id/events\nAuthorization: Bearer pf_live_...\nAccept: text/event-stream'} label="curl" />
    ...
    if (activeId === 'webhook-signatures') return (
      <div className="space-y-6">
        ...
        <Sub title="Signature format">
          <Code code={'X-Polyforge-Signature: sha256=<hex-digest>'} label="curl" />
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-pf-border overflow-hidden">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-pf-overlay/40 border-b border-pf-border">
                <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Header</th>
                <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Value</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={h} className={i < arr.length - 1 ? 'border-b border-pf-border-subtle' : ''}>
                  <td className="px-4 py-2.5 font-mono text-pf-cyan-300">{h}</td>
                  <td className="px-4 py-2.5 text-pf-text-secondary">{v}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="text-xs text-pf-text-muted bg-pf-overlay/40 rounded-lg px-3 py-2.5 border border-pf-border-subtle">
          Certain write endpoints (register, forgot-password) have tighter per-IP limits.
        ...
        <Sub title="HTTP status codes">
          <div className="rounded-lg border border-pf-border overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-pf-overlay/40 border-b border-pf-border">
                  <th className="text-left px-4 py-2.5 w-16 font-semibold text-pf-text-muted">Code</th>
                  <th className="text-left px-4 py-2.5 font-semibold text-pf-text-muted">Meaning</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={code} className={i < arr.length - 1 ? 'border-b border-pf-border-subtle' : ''}>
                    <td className="px-4 py-2.5 font-mono text-pf-text font-medium">{code}</td>
                    <td className="px-4 py-2.5 text-pf-text-secondary">{meaning}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Sub title="Example prompts">
          <div className="space-y-2">
            {[
              ...
              <p key={p} className="text-xs text-pf-text-secondary bg-pf-surface border border-pf-border rounded-lg px-3.5 py-2.5 font-mono">{p}</p>
            ))}
          </div>
        </Sub>
      </div>
  ...
  return (
    <div className="flex h-screen animate-fade-in">
      ...
      {/* ── Sidebar ── */}
      <aside className="hidden lg:flex flex-col w-64 shrink-0 border-r border-pf-border-subtle bg-pf-surface overflow-y-auto">

        {/* Branding header */}
        <div className="px-5 py-5 border-b border-pf-border-subtle shrink-0">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-bold text-pf-text tracking-tight">API Reference</span>
            <span className="text-[10px] font-mono bg-pf-cyan-500/15 text-pf-cyan-400 px-2 py-0.5 rounded-md ring-1 ring-pf-cyan-500/30">v1</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-pf-success shrink-0" />
            <code className="text-[10px] font-mono text-pf-text-muted truncate">api.polyforge.app</code>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-5">
          {NAV_GROUPS.map(g => (
            <div key={g.group ?? 'overview'}>
              {g.group && (
                <p className="text-[10px] font-semibold uppercase tracking-widest text-pf-text-muted mb-2 px-2">{g.group}</p>
              )}
              <div className="space-y-0.5">
                {g.items.map(item => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg text-[13px] transition-all duration-150 cursor-pointer border-l-2 ${
                      activeId === item.id
                        ? 'border-pf-cyan-500 bg-pf-cyan-500/10 text-pf-cyan-300 font-medium'
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t border-pf-border-subtle shrink-0">
          <Link to="/settings" className="text-[11px] text-pf-text-muted hover:text-pf-text transition-colors block">
            Settings → API Keys
          </Link>
        </div>
      </aside>
      ...
      <div ref={contentRef} className="flex-1 min-w-0 overflow-y-auto bg-pf-base">
        ...
        {/* Breadcrumb bar */}
        <div className="sticky top-0 z-10 flex items-center gap-2 px-8 py-3 bg-pf-base/90 backdrop-blur-sm border-b border-pf-border-subtle text-xs text-pf-text-muted">
          <span>Docs</span>
          <ChevronRight className="w-3 h-3 shrink-0" />
          {currentGroup && <><span>{currentGroup}</span><ChevronRight className="w-3 h-3 shrink-0" /></>}
          <span className="text-pf-text">{currentLabel}</span>
        </div>
