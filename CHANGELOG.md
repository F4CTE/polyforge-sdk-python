...
### Added
- **ops: Sentry on 5 services + Redis Stream lag monitoring + PEL reclaim worker (POLA-1968 / GitHub #1178)** — Added `@sentry/nestjs` instrumentation (`src/instrument.ts`, `SentryModule.forRoot()`, `SentryGlobalFilter`) to **backtest-service**, **bot-service**, **market-data-service**, **notification-service**, and **paper-order-service** — these were the only NestJS services missing Sentry; gateway is nginx so does not apply. Added `StreamMonitorService` and `PelReclaimService` to `@polyforge/shared-redis` and wired them into all four Redis Stream consumers (order-service `stream:orders`, paper-order-service `stream:paper_orders`, backtest-service `stream:backtests`, notification-service `stream:events`). The monitor polls XLEN/XPENDING/XINFO every 30s and emits structured warnings + Sentry breadcrumbs when length, pending count, or oldest-pending age cross thresholds. The reclaim worker uses `XAUTOCLAIM` (Redis 6.2+) every 60s with a 5-minute idle threshold, optionally re-runs the consumer's handler on each reclaimed entry, and ACKs only on success so transient failures do not lose data. New helpers `getStreamLag` and `reclaimPendingEntries` exported from `@polyforge/shared-redis` with unit coverage (15 new tests in `streams.spec.ts`, `pel-reclaim.service.spec.ts`, `stream-monitor.service.spec.ts`). Sentry is opt-in via `SENTRY_DSN`; the breadcrumb path uses a runtime require so processes without Sentry initialised are silent no-ops.
- **user-app: shared `ConfirmDialog` primitive (POLA-1935 PR #1 / GitHub #1182)** — Foundation for the four-PR trading-safety UX rollout. `apps/user-app/src/components/ui/confirm-dialog.tsx` wraps the `@polyforge/ui` Dialog primitives with WCAG 3.3.4-aligned defaults: Cancel as default focus on open, Escape cancels, both buttons explicitly `type="button"` so Enter never submits a host form. Optional `delayMs` defers confirm-button enablement (`AC1` live-trade flow needs `>= 2000` ms; default `0`). Optional `tone="danger"` escalates the dialog to `role="alertdialog"` so assistive tech treats the announcement as time-critical (per WAI-ARIA Authoring Practices). Description is wired through `aria-describedby` with a `useId`-generated id so multiple dialogs on a page do not collide. Loading state disables both buttons and shows a spinner. 14 Vitest tests cover render content, default-focus, click + Escape paths, no-Confirm-on-Cancel-Enter, delay-to-accept gating + countdown label + reset on reopen, loading-state disabling, role escalation, and aria-describedby linkage. No callsites converted yet — adoption (live-trade confirm + order-cancel confirm) lands in PR #2.

### Security
- **ops: Agent PR creation rate limits to prevent CI flood recurrence (POLA-5869)** — Added hard PR creation rate limits for all agents in `CLAUDE.md`, `AGENTS.md`, and `POLYFORGE_CEO_PROMPT.md`: max 10 PRs per rolling 60-minute window, max 3 PRs per session/heartbeat, mandatory batching of related fixes, and CEO/board approval gate for 4+ PRs. These complement the CI-level serialization (`ci-pr-queue` concurrency group) applied in POLA-5860. Agents observed exceeding the limits must be paused for rate-limit review.
- **infra/docker: prod network isolation gaps + .env in build context + gateway healthcheck (POLA-1973 / GitHub #1176)** —
  - **Prod (`docker-compose.prod.yml`):** removed `public` from `auth-service` (now `[internal, signer-only]`) and `api-service` (now `[internal]`) so backend services can no longer be reached on the bridge that hosts the gateway, eliminating the bypass of nginx rate-limiting and security headers. Added `internal` to `gateway`'s networks so it can still reach the upstreams nginx proxies to (api-service:3002, auth-service:3001).
  - **Gateway healthcheck:** the prod compose healthcheck (`wget http://localhost/healthz`) was hitting the HTTP→HTTPS redirect (`301`) and never returning a 2xx, which would have flapped the container as unhealthy. Added `location = /healthz { return 200; }` on the port-80 server in `services/gateway/nginx.prod.conf` (with `default_server` so the `localhost` Host header matches) — keeps healthcheck on plain HTTP, no TLS dance, no `--no-check-certificate` needed.
  - **Build context (`.dockerignore`):** added `.env`, `.env.*`, `**/.env`, `**/.env.*` exclusions (with `!.env.example`/`!*.example` allow-back) so a future `COPY .` cannot bake secrets into image layers.
  - **Dev signer/admin isolation (`docker-compose.infra.yml`):** added `internal: true` to the dev `signer-only` and `admin-only` networks. Dev `internal` is intentionally left as a regular bridge (services on it need outbound to Polymarket/Stripe/SES); the change closes the dev/prod parity gap specifically for credential-handling and admin-plane traffic. All members of `signer-only`/`admin-only` that legitimately need egress are also attached to `internal`, so outbound paths are preserved.
  - **PostHog secret default (`docker-compose.infra.yml`):** replaced `${POSTHOG_SECRET_KEY:-<GENERATE_ME>}` (in both `posthog` and `posthog-worker`) with `${POSTHOG_SECRET_KEY:?...}` so an unset secret fails compose at startup instead of silently booting with a literal placeholder string.
  - **@hono/node-server CVE (GHSA-92pp-h63x-v22m) — middleware bypass via repeated slashes in serveStatic (closes #1375)** — Added `@hono/node-server ^1.19.13` to `pnpm.overrides` in root `package.json`. The advisory impacts the Prisma dev server path (`prisma@7.8.0` → `@prisma/dev@0.24.3` → `@hono/node-server`); the override forces resolution to `1.19.14` (latest v1.x patch with the fix) without requiring a Prisma downgrade. The caret constraint avoids v2.0.0's breaking changes (Node v18 dropped, Vercel adapter removed). Verified: `pnpm audit` clean (0 advisories), typecheck passes (22/22), api-service tests pass (1390/1390), no application code uses `serveStatic`.

### Fixed
- **infra/elasticache: Redis HA IaC was un-appliable — restored Multi-AZ failover (POLA-1982 / GitHub #1156)** — A prior migration to `aws_elasticache_replication_group` left dangling references to the deleted `aws_elasticache_cluster.main` resource in `outputs.tf`, `secrets.tf`, and `cloudwatch.tf`, causing `terraform plan` to fail with "Reference to undeclared resource" and silently leaving production Redis on whatever was last applied. Repointed all references to `aws_elasticache_replication_group.main`; `REDIS_URL` now uses `primary_endpoint_address` so DNS swings transparently on failover; `outputs.tf` exposes both primary and reader endpoints. Added per-node CloudWatch alarms via `for_each` over deterministic member cluster IDs (`<rg-id>-001`, `<rg-id>-002`) so alarms work on first apply without "known after apply" errors. New alarms: `EngineCPUUtilization > 75%` (more accurate than `CPUUtilization` for Redis's single engine thread), `ReplicationLag > 10s` (RPO degradation detection). The replication group now publishes node lifecycle events (failover started/completed, replica promoted, snapshot success/failure, maintenance) to the existing alerts SNS topic via `notification_topic_arn`; the topic policy was updated to grant `elasticache.amazonaws.com` permission to publish. Dashboard updated to show per-node metrics for memory, connections, replication lag, and engine CPU. `docs/04-database-and-redis.md` documents the HA topology (RTO ≤ 2 min, RPO ≤ 1 s) and clarifies that ioredis transparently handles failover via DNS swing — services do not need Sentinel/Cluster client mode. `docs/ops/05-incident-response.md` adds a "Redis primary failover" runbook with detection signals, a 5-minute observation window, manual-recovery escalation steps, and post-incident retro requirements.
- **a11y: WCAG 2.1 AA violations — focus trap, chart alternatives, contrast, announcements (POLA-1965 / GitHub #1183)** — Multi-surface accessibility pass. (1) Added shared `useFocusTrap` hook in `@polyforge/ui` and wired it into the custom `<Dialog>` and the mobile navigation drawer so Tab/Shift-Tab cycle inside the modal and focus is restored to the trigger on close (WCAG 2.4.3 / 2.1.2). (2) Added `<LoadingAnnouncer>` component (visually-hidden polite live region) and used it in `<StrategyChart>` so screen readers hear loading/loaded transitions (WCAG 4.1.3). (3) `<StrategyChart>` now exposes an `aria-label` summary (latest price + percentage change + trade-marker count), a `role="img"` chart body, and an `sr-only` data-table fallback with full price series + trade markers (WCAG 1.1.1, 1.4.1). (4) `--text-tertiary` bumped from `#545770` (3.2:1 — failing AA) to `#7E8298` (~5.0:1) on dark and from `#9094AA` (2.9:1) to `#65687D` (~5.2:1) on light, both above the 4.5:1 AA floor while staying below `--text-secondary` so visual hierarchy is preserved (WCAG 1.4.3). (5) Mobile navigation drawer now keeps Tab inside the drawer while open, returns focus to the menu trigger on close, and exposes `aria-expanded` / `aria-controls` on the trigger (WCAG 2.4.3). (6) Onboarding `ProgressDots` switched from `role="tablist"`/`role="tab"` (non-functional tabs) to a single `role="progressbar"` with `aria-valuenow` / `aria-valuetext` (WCAG 4.1.2). (7) Step-3 `secondaryPath` corrected from `/strategies/builder` (404) to `/strategies/new`. 7 new unit tests covering the focus trap behaviour and the loading announcer. Out of scope, tracked separately: keyboard alternative for React Flow block wiring (HIGH #4 in the source audit).
- **api-service/arbitrage: harden `/arbitrage/execute` and `/arbitrage/positions/:id/close` against double-tap and silent 404s (POLA-1911)** — Surfaced during the POLA-1909 review of the 4 cross-venue arb SDK PRs (`polyforge-mcp#208`, `polyforge-sdk-ts#195`, `polyforge-sdk-python#209`, `polyforge-sdk-rust#188`). Both money-moving routes now (1) require an `Idempotency-Key` header (8–128 chars) via `IdempotencyInterceptor`, (2) carry `@Throttle({ limit: 5/min in production, ttl: 60_000ms })` — tighter than the orders bucket because each arb call opens 4 live orders across 2 venues, and (3) `ExecuteArbDto.matchId` is now validated as a UUID instead of a free-form string, returning a clear 400 instead of a silent `findUnique` → 404. New `arbitrage.controller.spec.ts` enforces interceptor + Throttle metadata on both routes and DTO-level UUID validation. Sweep-close semantics (GTC orders priced at `0.001` SELL / `0.999` BUY = market-equivalent sweep, not a resting limit) are now documented in `docs/06-api-catalog.md` and on the `closePosition` service JSDoc; SDK docstring updates tracked separately as child issues per repo.
...
### Added
- **frontends: PostHog JS SDK analytics across user-app, admin-app, and landing (POLA-1029)** — `posthog-js@^1.250.0` installed in all three frontends. `src/lib/analytics.ts` wrapper (no-op guard when key unset) created for both Vite apps. `page_viewed` fires on every React Router transition; `posthog.identify`/`reset` hooked into auth stores on login/logout; `strategy_form_opened` fires on new-strategy mount; landing `cta_clicked` events on hero and CTA banner anchor tags. PostHog provider wraps Next.js landing layout with `maskAllInputs: true` session replay. `VITE_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_KEY` added to `.env.example` (commented out — no keys committed).
...
### Security
- **auth: remove email (PII) from JWT payloads to fix GDPR log-exposure risk (POLA-1978 / GitHub #1160)** — JWT access tokens for both end users (`auth-service`) and admins (`admin-auth-service`) previously embedded `email` in the signed payload. Because JWTs are routinely surfaced in `Authorization` headers, cookies, reverse-proxy access logs, APM traces, browser dev-tools, and third-party SDKs, embedding email amounted to broadcasting PII into log destinations that were never meant to hold it. Fix: dropped `email` from `JwtPayload` and `AdminJwtPayload` in `@polyforge/shared-types`; removed `email` from token issuance in `services/auth-service/.../auth.service.ts#generateAccessToken` and `services/admin-auth-service/.../auth.service.ts#login`; removed `email` from the API-key path's synthesized `request.user` in `packages/shared-auth/src/jwt-auth.guard.ts`; removed `email` from `Sentry.setUser(...)` in `services/api-service/.../sentry-context.interceptor.ts`. Server-side flows that need the email continue to fetch it from the database via `sub` (the only PII-free identifier in the token). Login/me response bodies still include email — those are one-shot HTTPS responses, not stored credentials. Added regression tests in `auth.service.spec.ts` (user) and `admin-auth-service/auth.service.spec.ts` (admin) asserting `email` is not present in the signed payload.
- **api/auth: idempotency + DTO validation + Prisma error masking (POLA-1967 / GitHub #1179)** — Closes four API hygiene gaps surfaced by Ultra Review Pass 2.
- **admin-auth: explicit rate limiting on TOTP setup/confirm/disable endpoints (POLA-1977 / GitHub #1161)** — Although the global `ThrottlerGuard` was already registered in `app.module.ts` (closes #59), the three TOTP endpoints relied on the lenient default 10/15min bucket. Added explicit `@Throttle()` decorators: `disableTotp` and `confirmTotp` capped at **5 attempts / 15 min** (≪ the 1M-key brute-force space for the 6-digit code), and `setupTotp` capped at **10 / hour** to deter secret-rotation abuse. Introduced `services/admin-auth-service/src/common/throttle-limit.ts` mirroring the user-side helper — it returns the strict prod limit only when `NODE_ENV=production && CI!=true`, so parallel E2E shards aren't 429'd. Added 5 decorator-coverage regression tests (Reflect-metadata) on `AuthController` plus 3 helper tests (46 total tests passing). Swagger now documents `429` responses on all three TOTP endpoints.
- **api-service: per-user WebSocket connection cap on `/ws` (POLA-1979 / GitHub #1159)** — `EventsGateway.handleConnection` enforces `WS_MAX_CONNECTIONS_PER_USER` (default `5`). After successful JWT verification the gateway checks the size of the user's existing socket set; when the cap is reached, the oldest socket is closed with code `4008` and reason `Connection limit exceeded`, then the replacement connection is accepted. Prevents a single authenticated user or runaway reconnect loop from exhausting file descriptors/event-loop capacity in the api-service. Slot cleanup still runs on `handleDisconnect`. Gateway unit tests cover cap acceptance, oldest-socket replacement, slot reclaim on disconnect, per-user isolation, and default fallback. New env var documented in `docs/08-env-reference.md`.
- **order-service + signer-service: tighten DTO validation on order/close payloads (POLA-1975 / GitHub #1163)** — `ClosePositionDto.size` now requires `@Matches(/^\d+(\.\d+)?$/)` plus `@MaxLength(30)`, rejecting negatives (`"-100"`), scientific notation (`"1e10"`), hex (`"0x10"`), `"NaN"`, `"Infinity"`, leading/trailing whitespace, and pathologically long strings before they reach the service layer. `SignOrderDto.price` now carries `@Max(1)` in addition to the existing `@Min(0)`, enforcing the Polymarket probability domain (0..1) at the DTO boundary so a rogue `price=999` is rejected by Nest's `ValidationPipe`. Added 12 unit tests for `ClosePositionDto.size` and 6 unit tests for `SignOrderDto.price` covering accept/reject/boundary cases.
- **gateway: removed `'unsafe-inline'` and `'unsafe-eval'` from dev CSP `script-src` (POLA-1976 / GitHub #1162)** — `apps/gateway/nginx.ssl.conf` and `services/gateway/nginx.dev.conf` previously emitted `script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net`, which neutered XSS protections in dev (and via container image promotion paths). Hardened to `script-src 'self' 'wasm-unsafe-eval'` to match the production posture (`services/gateway/nginx.prod.conf` already enforces `'self'` only). The narrower `'wasm-unsafe-eval'` token is kept as a hedge for any future browser-side WASM (Rust strategy/crypto modules currently run server-side). `'unsafe-inline'` is retained on `style-src` only — Tailwind/Vite/shadcn emit dynamic `<style>` tags at runtime; nonce/hash migration tracked separately. The `cdn.jsdelivr.net` allowance was dropped because it was only needed by the api-service Swagger UI page, which sets its own CSP for that response.

### Fixed
- **auth-service: refresh-token replay regression coverage (POLA-1300 / GitHub #1025)** — Added unit coverage proving concurrent `AuthService.refresh()` calls sharing the same refresh token cannot both rotate successfully. The first caller receives a replacement token; the replayed caller receives `401 REFRESH_TOKEN_REPLAY` before any JWT or replacement refresh token is issued. API docs now document the rotated `pf_refresh` cookie and replay error.
- **user-app: New Strategy CTA invisible at 375px mobile viewport (POLA-987 / GitHub #1001)** — Header row previously used `flex items-center justify-between` with no mobile stacking, causing the three-button group (Compare + Import Strategy + New Strategy) to overflow beyond the `overflow-hidden` root and become completely clipped. Fix: changed to `flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3` so the button row stacks below the title on mobile; added `flex-wrap` to the button group so individual CTAs wrap gracefully. Verified at 375px (iPhone SE) and 1280px (desktop — no regression). (landed in PR #1003)

## [Unreleased] — 2026-04-25

### Added
- **user-app: ComboMarketCard — Kalshi combo/multivariate market picker for strategy builder (POLA-953)** — New `ComboMarketCard` React component in `apps/user-app/src/components/builder/combo/` that guides users through a 3-phase browse → detail → confirm flow for selecting Kalshi combo markets. Phase 1 (browse): fetches `GET /api/v1/markets/combo/collections`, renders collection cards with status badges (active/settled/expired). Phase 2 (detail): collapsible leg breakdown panel, inline leg adder with duplicate-ticker guard, calls `POST /api/v1/markets/combo/lookup` to resolve the pre-built `market_ticker`. Phase 3 (confirm): shows resolved ticker with a "Use this market" CTA that emits the ticker back to the block node. Integrated into `block-node.tsx` via a new `combo_slot` field type — renders as a compact trigger button when closed, expands inline when open. New `combo_leg` action block added to `block-definitions.ts` (Kalshi group) with `marketTicker`, `side`, `size`, and `orderType` fields. New `ComboCollection`, `ComboLeg`, `ComboLookupResult` interfaces added to `@polyforge/shared-types`. `COMBO_LEG` added to `BlockType` enum. 24 unit tests covering all phases, error states, accessibility, and navigation.
- **user-app: Liquidity Rewards card on market detail page + sponsored-markets in portfolio (POLA-955)** — New `MarketRewardsCard` component renders on every market detail page; fetches from `GET /api/v1/rewards/market/:marketId` (daily pool, remaining reward with progress bar, LP requirements, APY estimate) with loading skeleton and graceful no-rewards state. "Sponsor this Market" CTA calls `GET /api/v1/rewards/sponsor-url/:marketId` and opens Polymarket in a new tab. Enhanced `RewardsDashboard` (portfolio page) with a new Sponsored Markets section backed by `GET /api/v1/rewards/user/sponsored-markets`; list refreshes on window-focus event and on a 30-second polling interval while the page is in the foreground. 25 new unit tests added.
...
### Fixed
- **Design: replace 367 `rounded-xl`/`rounded-lg` with charter-compliant `rounded-pf` (POLA-263, closes #768)** — batch-migrated all 92 files in user-app and admin-app; replaced directional variants (`rounded-t-xl` → `rounded-t-pf`, `rounded-b-xl` → `rounded-b-pf`); removed deprecated `--radius-lg` and `--radius-xl` CSS custom properties from `packages/ui/src/globals.css` now that zero usages remain
...
### Added
- **landing: /privacy page (POLA-168)** — creates `apps/landing/app/privacy/page.tsx`; fixes 404 for footer Privacy Policy link. Page follows Design Charter §2 type scale with Nav + Footer wrapper, max-w-3xl prose layout, and section dividers.
...
### Fixed (UX)
- **Mobile sidebar not closing on navigation** — added `useLocation` effect to auto-close sidebar on route change

### Fixed (E2E)
- **Copy-trading back button test** — fixed mode heading assertion
- **Copy-trading created copy in list test** — handle API rejection gracefully
...
- **#505/#506/#507/#508/#509/#510 Design batch** — news-feed sentiment colors tokenized, admin arbitrary sizing/hover/transitions fixed, missing min-w/min-h tokens added
- **#482 Hero leading-[1.15]** — replaced with `leading-pf-tight` token
...
### Fixed (Security)
- **Switch ThrottlerModule to Redis-backed storage for auth services (closes #393)** — `auth-service`, `admin-auth-service`, `api-service`, `admin-api-service`: replaced in-memory ThrottlerModule.forRoot() with ThrottlerModule.forRootAsync() using `@nest-lab/throttler-storage-redis`; rate-limit counters now persist across restarts and are shared across replicas, preventing brute-force bypass via container restart or load-balancer splitting
...
### Added
- **Checkbox, Switch, Label, Separator, and Toaster components (closes #375)** — `packages/ui/src/components/ui/`: added 5 missing shadcn/ui-style components to the shared UI package; Checkbox uses peer CSS for checked state with Lucide Check icon; Switch uses peer-checked translate for thumb animation; Label forwards ref with peer-disabled support; Separator supports horizontal/vertical orientation with proper ARIA roles; Toaster wraps Sonner with PolyForge design token defaults (`bg-pf-elevated`, `border-pf-border`, `text-pf-text`); all exported from `packages/ui/src/index.ts`
...
### Fixed (Design / Tokens)
- **Replace non-standard CSS transition durations with tokens (closes #250)** — `user-app/globals.css`: React Flow minimap `0.2s` → `var(--duration-pf-normal)`, controls button `0.15s` → `var(--duration-pf-fast)`; `admin-app/globals.css`: skip-to-content `0.15s` → `var(--duration-pf-fast)`
- **Tokenize arbitrary OTP input letter-spacing (closes #251)** — added `--tracking-pf-code: 0.3em` token to `packages/ui/src/globals.css`; replaced `tracking-[0.3em]` with `tracking-[var(--tracking-pf-code)]` in both `user-app` and `admin-app` login pages
- **Replace forbidden .5 spacing in admin revenue page and user portfolio (closes #255)** — `admin-app/src/pages/revenue/revenue.tsx` and `user-app/src/pages/portfolio/portfolio.tsx`: `gap-y-1.5` (6px) → `gap-y-2` (8px) per 4px-grid rule
- **Replace arbitrary border-t-[3px] in admin sidebar (closes #254)** — `admin-app/src/components/layout/admin-sidebar.tsx`: `border-t-[3px]` → `border-t-4` (standard Tailwind 4px width)

### Fixed (Design / Code Quality)
- **Remove unused design tokens from globals.css (closes #377)** — removed 20 dead CSS custom properties: `--color-pf-gold-glow`, `--color-pf-purple-glow`, `--color-pf-text-disabled`, `--color-pf-chart-1` through `--color-pf-chart-6`, `--color-pf-chart-muted`, and `--spacing-pf-1` through `--spacing-pf-10`; these tokens had zero references in any component, page, or service file; kept all tokens with actual usage (chart-grid, chart-tooltip-bg/border, all shadows, all font-sizes, duration-normal, status colors, etc.)
...
- **Pin PgBouncer Docker image to specific version (closes #230)** — `docker-compose.prod.yml`: both `pgbouncer` and `pgbouncer-admin` images pinned from `edoburu/pgbouncer:latest` to `edoburu/pgbouncer:v1.23.1-p3`; eliminates silent upgrade risk on `docker compose pull`
