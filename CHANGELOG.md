...
- **infra/docker: prod network isolation gaps + .env in build context + gateway healthcheck (POLA-1973 / GitHub #1176)** —
  ...
  - **@hono/node-server CVE (GHSA-92pp-h63x-v22m) — middleware bypass via repeated slashes in serveStatic (closes #1375)** — Added `@hono/node-server >=1.19.13` to `pnpm.overrides` in root `package.json`. The advisory impacts the Prisma dev server path (`prisma@7.8.0` → `@prisma/dev@0.24.3` → `@hono/node-server`); the override forces resolution to `2.0.0` (>1.19.13) without requiring a Prisma downgrade. Verified: `pnpm audit` clean (0 advisories), build and typecheck pass, no application code directly uses `serveStatic`.
