...
const BOT_JWT_SECRET  = process.env.BOT_JWT_SECRET  ?? 'dev-bot-jwt-secret-change-in-production';
...
export class LinkingService {
     ...
     */
    async getUserId(channel: BotChannelType, chatId: string): Promise<string | null> {
     ...
     */
    async connect(channel: BotChannelType, chatId: string, code: string): Promise<string> {
        // Already linked?
        ...
        const token = this.jwt.sign(tokenPayload, { secret: BOT_JWT_SECRET, expiresIn: BOT_JWT_EXPIRES });
        ...
        await this.prisma.botConnection.create({
            data: { userId, channel: channel as any, chatId, tokenHash, active: true },
        });
