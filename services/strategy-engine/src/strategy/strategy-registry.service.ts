...
export class StrategyRegistryService {
    ...
    async pause(strategyId: string): Promise<void> {
        ...
        runner.pause('manual');
        await this.prisma.strategy.update({ where: { id: strategyId }, data: { status: StrategyStatus.PAUSED } });
        await this.emitEvent(strategyId, await this.getUserId(strategyId), 'STRATEGY_STOPPED', 'manual pause');
    }
    ...
    async stop(strategyId: string): Promise<void> {
        ...
        await this.prisma.strategy.update({ where: { id: strategyId }, data: { status: StrategyStatus.IDLE } });
        await this.emitEvent(strategyId, await this.getUserId(strategyId), 'STRATEGY_STOPPED', 'manual stop');
    }
    ...
    private getRunner(strategyId: string): StrategyRunner {
        const runner = this.runners.get(strategyId);
        if (!runner) throw new NotFoundException(`Strategy ${strategyId} is not running`);
        return runner;
    }
    ...
    private async emitEvent(strategyId: string, userId: string, type: string, reason?: string) {
    ...
    private async getUserId(strategyId: string): Promise<string> {
        const s = await this.prisma.strategy.findUnique({ where: { id: strategyId }, select: { userId: true } });
        return s?.userId ?? '';
    }
}
