...
export class StrategyRunner {
    ...
    constructor(
        ...
        private readonly onStatusChange: (status: StrategyRunnerStatus, reason?: string) => Promise<void>,
    ) {
    ...
    private async evaluate() {
        ...
        if (staleToken) {
            if (this.status === 'RUNNING') {
                ...
                await this.emitStrategyEvent('STRATEGY_PAUSED', `stale_market_data:${staleToken}`);
            }
            return;
        }

        // Auto-resume from stale pause when data is fresh again
        if (this.status === 'PAUSED' && this.pauseReason?.startsWith('stale_market_data')) {
            this.resume();
        ...
        for (const block of this.safety) {
            ...
            const result = await evaluator.evaluate(block as any, ctx, this.redis, this.prisma);
            if (!result.fired) {
                this.stop();
        ...
        for (const block of this.actions) {
            ...
            const result = await evaluator.execute(block as any, ctx, this.redis, this.prisma);
