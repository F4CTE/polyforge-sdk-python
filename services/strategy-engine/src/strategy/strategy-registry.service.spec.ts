...
function makeRedisMock(overrides: Record<string, unknown> = {}) {
    return {
        ...
        getClient: vi.fn().mockReturnValue({ xadd: vi.fn().mockResolvedValue('1-0') }),
        ...overrides,
    } as any;
}
...
function makeDbStrategy(overrides: Partial<{
    id: string;
    userId: string;
...
describe('StrategyRegistryService — start()', () => {
    ...
    it('updates strategy status to RUNNING for a TICK strategy', async () => {
        const strategy = makeDbStrategy({ status: StrategyStatus.IDLE, execMode: 'TICK' });
        prisma.strategy.findUnique.mockResolvedValue(strategy);
    ...
    it('keeps status as PAPER for a paper strategy', async () => {
        const strategy = makeDbStrategy({ status: StrategyStatus.PAPER, execMode: 'TICK' });
        prisma.strategy.findUnique.mockResolvedValue(strategy);
    ...
    it('emits STRATEGY_STARTED event to stream:events', async () => {
        ...
        expect(redis.xadd).toHaveBeenCalledWith(
            'stream:events',
            expect.objectContaining({ type: 'STRATEGY_STARTED', strategyId: 'strat-1' }),
        );
    });
...
describe('StrategyRegistryService — stop()', () => {
    ...
    it('emits STRATEGY_STOPPED event', async () => {
        ...
        expect(redis.xadd).toHaveBeenCalledWith(
            'stream:events',
            expect.objectContaining({ type: 'STRATEGY_STOPPED', strategyId: 'strat-1' }),
        );
    });
...
describe('StrategyRegistryService — resume()', () => {
    ...
    it('runner status transitions back to RUNNING', async () => {
        ...
        prisma.strategy.findUnique.mockResolvedValue({ status: StrategyStatus.PAUSED });
    ...
    it('sets status to PAPER when resuming a paper strategy', async () => {
        ...
        prisma.strategy.findUnique.mockResolvedValue({ status: StrategyStatus.PAPER });
