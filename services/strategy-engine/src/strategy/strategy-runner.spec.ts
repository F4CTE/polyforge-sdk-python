...
function makeRedis(overrides: Record<string, unknown> = {}) {
    return {
        ...
        getClient: vi.fn().mockReturnValue({ lrange: vi.fn().mockResolvedValue([]) }),
...
function makePrisma(overrides: Record<string, unknown> = {}) {
    return {
        ...
        position: { findMany: vi.fn().mockResolvedValue([]), findUnique: vi.fn().mockResolvedValue(null) },
        token:    { findUnique: vi.fn().mockResolvedValue(null), findFirst: vi.fn().mockResolvedValue(null) },
        market:   { findFirst: vi.fn().mockResolvedValue(null), findUnique: vi.fn().mockResolvedValue(null) },
        ...overrides,
    } as any;
}

const DEFAULT_STATE = {
    betsToday: 0, dailyPnl: 0, consecutiveLoss: 0, consecutiveWin: 0,
    lastTradeAt: 0, tradedTokensToday: [], totalOrders: 0,
};
...
function makeRunner({
    ...
    prisma = makePrisma(),
    state = makeState(),
    onIntents = vi.fn<[OrderIntent[]], Promise<void>>().mockResolvedValue(undefined),
    onStatusChange = vi.fn().mockResolvedValue(undefined),
} = {}) {
    return new StrategyRunner(
        'strat-test', 'user-test', execMode, tickMs,
        triggers, conditions, actions, safety,
        redis, prisma, state, onIntents, onStatusChange,
    );
}
...
describe('StrategyRunner — stale data detection', () => {
    it('pauses and emits STRATEGY_PAUSED when price data is stale', async () => {
        ...
        expect(runner.status).toBe('PAUSED');
        expect(onStatusChange).toHaveBeenCalledWith('PAUSED', expect.stringContaining('stale_market_data'));
        expect(redis.xadd).toHaveBeenCalledWith('stream:events', expect.objectContaining({ type: 'STRATEGY_PAUSED' }));
    });
...
describe('StrategyRunner — SAFETY evaluation', () => {
    it('stops strategy and emits STRATEGY_STOPPED when safety block fails', async () => {
        ...
        expect(runner.status).toBe('STOPPED');
        expect(onStatusChange).toHaveBeenCalledWith('STOPPED', expect.stringContaining('SAFETY STOP'));
        expect(prisma.strategy.update).toHaveBeenCalledWith(
            expect.objectContaining({ data: { status: 'IDLE' } }),
        );
        expect(redis.xadd).toHaveBeenCalledWith('stream:events', expect.objectContaining({ type: 'STRATEGY_STOPPED' }));
    });
    ...
    it('does not stop when all safety blocks pass', async () => {
        ...
        expect(runner.status).toBe('RUNNING');
        expect(onStatusChange).not.toHaveBeenCalledWith('STOPPED', expect.anything());
    });
});
...
describe('StrategyRunner — CONDITION evaluation', () => {
    it('skips tick when a condition fails (AND logic)', async () => {
        ...
        const runner = makeRunner({
            ...
            state,
            onIntents,
            conditions: [{ id: 'c1', type: 'max_bets_per_day', params: { max: '3' } }],
        });
...
describe('StrategyRunner — ACTION execution + state update', () => {
    it('calls onIntents with produced OrderIntents', async () => {
        const state = makeState();
        const prisma = makePrisma();
        prisma.token.findUnique.mockResolvedValue({ id: 'tok-yes', marketId: 'mkt-1', outcome: 'YES' });
        const redis = makeRedis({ getJson: vi.fn().mockResolvedValue({ price: 0.7 }) });
        const onIntents = vi.fn().mockResolvedValue(undefined);
        ...
        const runner = makeRunner({
            ...
            prisma,
            onIntents,
            actions: [{ id: 'a1', type: 'buy_yes', params: { tokenId: 'tok-yes', size: '10' } }],
        });
...
describe('StrategyRunner — start() timer management', () => {
    ...
    it('enforces minimum tick interval of 200ms', () => {
        ...
        expect(() => { runner.start(); runner.stop(); }).not.toThrow();
    });
});
...
describe('StrategyRunner — stale data auto-resume path', () => {
    it('auto-resumes mid-tick when stale pause was set during the same evaluate() call chain', async () => {
        ...
        const runner = makeRunner({ execMode: 'EVENT', state, redis, onStatusChange });
