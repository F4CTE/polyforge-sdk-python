...
export class StateService {
    ...
    async update(strategyId: string, patch: Partial<StrategyState>): Promise<StrategyState> {
    ...
    async getPriceAge(tokenId: string): Promise<number> {
        ...
        try {
            const { timestamp } = JSON.parse(raw) as { price: number; timestamp: number };
    ...
    async getPrice(tokenId: string): Promise<{ price: number; timestamp: number } | null> {
        return this.redis.getJson<{ price: number; timestamp: number }>(`cache:price:${tokenId}`);
    }
