...
describe('StateService', () => {
    ...
    describe('get()', () => {
        ...
        it('merges stored state with defaults', async () => {
            redis.get.mockResolvedValue(JSON.stringify({ betsToday: 3, dailyPnl: -5 }));
            const state = await svc.get('strat-1');
    ...
    describe('update()', () => {
        it('merges a partial patch into the existing state', async () => {
            redis.get.mockResolvedValue(JSON.stringify({ ...DEFAULT_STATE, betsToday: 2, dailyPnl: -3 }));
    ...
    describe('getBook()', () => {
        it('delegates to redis.getJson with cache:book key', async () => {
            const book = { bids: [], asks: [], midpoint: '0.5', spread: '0.02', timestamp: 111 };
    ...
    describe('getPriceAge()', () => {
        ...
        it('returns approximate age when timestamp is stored', async () => {
            ...
            redis.get.mockResolvedValue(JSON.stringify({ price: 0.5, timestamp: ts }));
            const age = await svc.getPriceAge('tok-1');
