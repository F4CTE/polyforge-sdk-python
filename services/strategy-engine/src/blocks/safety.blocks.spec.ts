...
describe('StopIfDailyLossBlock', () => {
    it('fires when daily loss is within limit', async () => {
        ...
        const res = await StopIfDailyLossBlock.evaluate(block('stop_if_daily_loss', { maxLossUsdc: '10' }), ctx, makeRedis(), makePrisma());
    ...
    it('defaults maxLossUsdc to 0 when not provided', async () => {
        ...
        const res = await StopIfDailyLossBlock.evaluate(block('stop_if_daily_loss', {}), ctx, makeRedis(), makePrisma());
...
describe('StopIfOrdersPerMinBlock', () => {
    it('fires when order rate is below limit', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfOrdersPerMinBlock.evaluate(block('stop_if_orders_per_min', { maxOrders: '10' }), ctx, redis, makePrisma());
        expect(res.fired).toBe(true);
    });
    ...
    it('treats null redis value as 0 orders', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfOrdersPerMinBlock.evaluate(block('stop_if_orders_per_min', { maxOrders: '1' }), ctx, redis, makePrisma());
    ...
    it('uses correct Redis key with strategyId', async () => {
        ...
        const ctx = makeCtx();
        await StopIfOrdersPerMinBlock.evaluate(block('stop_if_orders_per_min', { maxOrders: '60' }), ctx, redis, makePrisma());
    ...
    it('defaults maxOrders to 60 when not provided', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfOrdersPerMinBlock.evaluate(block('stop_if_orders_per_min', {}), ctx, redis, makePrisma());
...
describe('StopIfConsecutiveLossBlock', () => {
    it('fires when consecutive losses are below limit', async () => {
        ...
        const res = await StopIfConsecutiveLossBlock.evaluate(block('stop_if_consecutive_loss', { maxLosses: '3' }), ctx, makeRedis(), makePrisma());
        expect(res.fired).toBe(true);
    });
...
describe('StopIfExposureExceedsBlock', () => {
    it('fires when total exposure is below limit', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfExposureExceedsBlock.evaluate(block('stop_if_exposure_exceeds', { maxUsdc: '20' }), ctx, makeRedis(), prisma);
    ...
    it('does NOT fire when exposure equals or exceeds limit', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfExposureExceedsBlock.evaluate(block('stop_if_exposure_exceeds', { maxUsdc: '40' }), ctx, makeRedis(), prisma);
        expect(res.fired).toBe(false);
        expect(res.reason).toMatch(/SAFETY STOP/);
    });
    ...
    it('fires when user has no positions (exposure = 0)', async () => {
        ...
        const ctx = makeCtx();
        const res = await StopIfExposureExceedsBlock.evaluate(block('stop_if_exposure_exceeds', { maxUsdc: '100' }), ctx, makeRedis(), prisma);
        expect(res.fired).toBe(true);
    });
    ...
    it('queries positions for the correct userId', async () => {
        const prisma = makePrisma();
        const ctx = makeCtx();
        await StopIfExposureExceedsBlock.evaluate(block('stop_if_exposure_exceeds', { maxUsdc: '100' }), ctx, makeRedis(), prisma);
...
describe('PauseAfterFillBlock', () => {
    it('fires when cooldown has elapsed', async () => {
        ...
        const res = await PauseAfterFillBlock.evaluate(block('pause_after_fill', { pauseMs: '5000' }), ctx, makeRedis(), makePrisma());
    ...
    it('fires when pauseMs is 0 (no cooldown)', async () => {
        ...
        const res = await PauseAfterFillBlock.evaluate(block('pause_after_fill', { pauseMs: '0' }), ctx, makeRedis(), makePrisma());
        expect(res.fired).toBe(true);
    });

    it('fires when lastTradeAt is 0 (never traded)', async () => {
        const ctx = makeCtx({ lastTradeAt: 0 });
        const res = await PauseAfterFillBlock.evaluate(block('pause_after_fill', { pauseMs: '60000' }), ctx, makeRedis(), makePrisma());
...
describe('MaxOrdersTotalBlock', () => {
    it('fires when total orders are below max', async () => {
        ...
        const res = await MaxOrdersTotalBlock.evaluate(block('max_orders_total', { max: '50' }), ctx, makeRedis(), makePrisma());
        expect(res.fired).toBe(true);
    });
    ...
    it('fires when totalOrders is 0', async () => {
        ...
        const res = await MaxOrdersTotalBlock.evaluate(block('max_orders_total', { max: '1' }), ctx, makeRedis(), makePrisma());
        expect(res.fired).toBe(true);
    });
});
