 ...
 */
function toNotifType(streamType: string, data: Record<string, string>): string | null {
    switch (streamType) {
        case 'ORDER_FILLED':
            return 'ORDER_FILLED';
        case 'STRATEGY_ERROR':
...
export class EventsConsumerService implements OnModuleInit, OnModuleDestroy {
    ...
    private async ensureGroup() {
        try {
            await this.redis.getClient().xgroup('CREATE', STREAM, GROUP, '$', 'MKSTREAM');
                    ...
                    for (const [id, fields] of messages) {
                        ...
                        if (notifType) {
                            try {
                                ...
                                this.logger.error(`Failed to handle notification ${notifType} for user ${event.userId}`, err);
                            }
                        }
