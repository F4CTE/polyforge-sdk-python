...
describe('TemplatesService', () => {
    ...
    describe('build', () => {
        ...
        describe('DAILY_LOSS_LIMIT', () => {
            it('returns correct title and error severity', () => {
                const result = service.build('DAILY_LOSS_LIMIT', { strategyId: 'strat-5' });
        ...
        describe('SOMEONE_FORKED', () => {
            ...
            it('uses "Someone" when forkerUsername absent', () => {
                const result = service.build('SOMEONE_FORKED', { strategyName: 'My Strategy' });
        ...
        describe('SOMEONE_FOLLOWED', () => {
            it('returns correct title and info severity', () => {
                const result = service.build('SOMEONE_FOLLOWED', { followerUsername: 'bob' });
        ...
        describe('SOMEONE_LIKED', () => {
            ...
            it('uses "Someone" when likerUsername absent', () => {
                const result = service.build('SOMEONE_LIKED', { strategyName: 'Alpha Strat' });
                expect(result.body).toContain('Someone');
            });
        });
        ...
        describe('SOMEONE_COMMENTED', () => {
            ...
            it('uses "Someone" when commenterUsername absent', () => {
                const result = service.build('SOMEONE_COMMENTED', { strategyName: 'Beta Strat' });
                expect(result.body).toContain('Someone');
            });
        });
    ...
    describe('toHtml', () => {
        it('returns a string containing the title', () => {
            const html = service.toHtml({ title: 'Test Title', body: 'Test body.', severity: 'info' });
            expect(html).toContain('Test Title');
        });

        it('returns a string containing the body text', () => {
            const html = service.toHtml({ title: 'T', body: 'Important message here.', severity: 'warning' });
            expect(html).toContain('Important message here.');
        });

        it('includes the Polyforge brand name', () => {
            const html = service.toHtml({ title: 'T', body: 'B', severity: 'success' });
        ...
        it('uses the warning accent colour (#f59e0b) for warning severity', () => {
            const html = service.toHtml({ title: 'T', body: 'B', severity: 'warning' });
