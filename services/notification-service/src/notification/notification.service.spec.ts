...
describe('NotificationService', () => {
    ...
    beforeEach(() => {
        ...
        service = new NotificationService(
            prisma, redis, mail, telegram, discord, templates,
        );
    });
    ...
    describe('handle', () => {
        ...
        it('returns early when the per-event opt-in flag is false', async () => {
            ...
            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.80', size: '100' });
            expect(templates.build).not.toHaveBeenCalled();
        });
        ...
        it('dispatches immediately when notificationFreq is IMMEDIATE', async () => {
            ...
            const dispatchSpy = vi.spyOn(service, 'dispatch').mockResolvedValue(undefined);
        ...
        it('enqueues digest when notificationFreq is HOURLY', async () => {
            ...
            const enqueueSpy = vi.spyOn(service, 'enqueueDigest').mockResolvedValue(undefined);

            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.80', size: '100' });

            expect(enqueueSpy).toHaveBeenCalledOnce();
            expect(enqueueSpy).toHaveBeenCalledWith(
                'user-1', 'HOURLY', 'ORDER_FILLED', expect.any(Object), STUB_CONTENT,
            );
        });
        ...
        it('enqueues digest when notificationFreq is DAILY', async () => {
            ...
            expect(enqueueSpy).toHaveBeenCalledOnce();
            expect(enqueueSpy).toHaveBeenCalledWith(
                'user-1', 'DAILY', 'ORDER_FILLED', expect.any(Object), STUB_CONTENT,
            );
        });
        ...
        it('always pushes an in-app notification regardless of frequency', async () => {
            ...
            expect(redis.xadd).toHaveBeenCalledWith('stream:events', expect.objectContaining({
                type: 'NOTIFICATION',
                userId: 'user-1',
            }));
        });
        ...
        it('skips ORDER_FILLED when fillUsdc is below minFillNotifyUsdc threshold', async () => {
            const prefs = makePrefs({ minFillNotifyUsdc: '100', notificationFreq: 'IMMEDIATE' });
            (prisma.notificationPreference.findUnique as any).mockResolvedValue(prefs);

            const dispatchSpy = vi.spyOn(service, 'dispatch').mockResolvedValue(undefined);

            // fillPrice 0.50 * size 10 = 5 USDC — below threshold of 100
            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.50', size: '10' });

            expect(dispatchSpy).not.toHaveBeenCalled();
        });

        it('dispatches ORDER_FILLED when fillUsdc meets or exceeds threshold', async () => {
            const prefs = makePrefs({ minFillNotifyUsdc: '10', notificationFreq: 'IMMEDIATE' });
            (prisma.notificationPreference.findUnique as any).mockResolvedValue(prefs);

            const dispatchSpy = vi.spyOn(service, 'dispatch').mockResolvedValue(undefined);

            // fillPrice 0.50 * size 100 = 50 USDC — above threshold of 10
            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.50', size: '100' });

            expect(dispatchSpy).toHaveBeenCalledOnce();
        });

        it('dispatches ORDER_FILLED when minFillNotifyUsdc threshold is 0 regardless of fill amount', async () => {
            const prefs = makePrefs({ minFillNotifyUsdc: '0', notificationFreq: 'IMMEDIATE' });
            (prisma.notificationPreference.findUnique as any).mockResolvedValue(prefs);

            const dispatchSpy = vi.spyOn(service, 'dispatch').mockResolvedValue(undefined);

            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.01', size: '1' });
        ...
        it('dispatches PRICE_ALERT even though it maps to null pref field (always send)', async () => {
            ...
            await service.handle('PRICE_ALERT', { userId: 'user-1', tokenId: 'tok-1', threshold: '0.9' });
        ...
        it('uses cached prefs from Redis when available', async () => {
            ...
            await service.handle('ORDER_FILLED', { userId: 'user-1', fillPrice: '0.80', size: '50' });
    ...
    describe('dispatch', () => {
        ...
        it('does not send email when emailEnabled is false', async () => {
            ...
            await service.dispatch('user-1', prefs, 'ORDER_FILLED', baseData, STUB_CONTENT);
        ...
        it('sends email when emailEnabled is true and user email exists', async () => {
            ...
            (prisma.user.findUnique as any).mockResolvedValue({ email: 'user@example.com' });
            (prisma.notificationHistory.create as any).mockResolvedValue({});
        ...
        it('sends telegram when telegramEnabled is true and chatId exists', async () => {
            ...
            (prisma.botConnection.findFirst as any).mockResolvedValue({ chatId: 'tg-chat-1' });
            (prisma.notificationHistory.create as any).mockResolvedValue({});
        ...
        it('sends discord when discordEnabled is true and channelId exists', async () => {
            ...
            (prisma.botConnection.findFirst as any).mockResolvedValue({ chatId: 'dc-chan-1' });
            (prisma.notificationHistory.create as any).mockResolvedValue({});
        ...
        it('sends to all enabled channels when all are enabled', async () => {
            const prefs = makePrefs({ emailEnabled: true, telegramEnabled: true, discordEnabled: true });
            (prisma.user.findUnique as any).mockResolvedValue({ email: 'u@example.com' });
            (prisma.botConnection.findFirst as any).mockResolvedValue({ chatId: 'some-id' });
            (prisma.notificationHistory.create as any).mockResolvedValue({});
        ...
        it('writes notification history with success=false when telegram.send throws', async () => {
            ...
            await expect(
                service.dispatch('user-1', prefs, 'ORDER_FILLED', baseData, STUB_CONTENT),
            ).resolves.toBeUndefined();

            expect(prisma.notificationHistory.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ channel: 'TELEGRAM', success: false }),
                }),
            );
        });
    });
    ...
    describe('enqueueDigest', () => {
        it('rpushes a JSON item to the correct hourly key', async () => {
            await service.enqueueDigest('user-42', 'HOURLY', 'ORDER_FILLED', {}, STUB_CONTENT);
        ...
        it('rpushes a JSON item to the correct daily key', async () => {
            await service.enqueueDigest('user-42', 'DAILY', 'BACKTEST_COMPLETE', {}, STUB_CONTENT);
        ...
        it('sets a TTL on the digest key', async () => {
            ...
            expect(redisClient.expire).toHaveBeenCalledWith('digest:hourly:user-42', 90000);
        });

        it('serialises title and body into the queued item', async () => {
            await service.enqueueDigest('user-42', 'DAILY', 'ORDER_FILLED', {}, STUB_CONTENT);
    ...
    describe('flushDigest', () => {
        ...
        it('does nothing for a key whose lrange is empty', async () => {
            ...
            (prisma.notificationPreference.findUnique as any).mockResolvedValue(makePrefs());
        ...
        it('scans with correct pattern for HOURLY', async () => {
            ...
            expect(redisClient.scan).toHaveBeenCalledWith('0', 'MATCH', 'digest:hourly:*', 'COUNT', '100');
        });
        ...
        it('scans with correct pattern for DAILY', async () => {
            ...
            expect(redisClient.scan).toHaveBeenCalledWith('0', 'MATCH', 'digest:daily:*', 'COUNT', '100');
        });
        ...
        it('uses hourly digest subject format for HOURLY frequency', async () => {
            const prefs = makePrefs({ emailEnabled: true });
            redisClient.scan.mockResolvedValueOnce(['0', ['digest:hourly:user-1']]);
            redisClient.lrange.mockResolvedValueOnce([makeDigestItem(), makeDigestItem()]);
            (prisma.notificationPreference.findUnique as any).mockResolvedValue(prefs);
            (prisma.user.findUnique as any).mockResolvedValue({ email: 'u@example.com' });
            (prisma.notificationHistory.create as any).mockResolvedValue({});
            ...
            const [, subject] = (mail.send as any).mock.calls[0] as [string, string, ...unknown[]];
