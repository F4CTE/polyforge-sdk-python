...
export class NotificationService {
    ...
    async handle(eventType: string, data: Record<string, string>): Promise<void> {
        ...
        if (prefField !== null && !(prefs as any)[prefField]) return;

        // For ORDER_FILLED: check minFillNotifyUsdc threshold
        if (eventType === 'ORDER_FILLED') {
            const fillUsdc = parseFloat(data.fillUsdc ?? data.fillPrice ?? '0') *
        ...
        if (freq === 'IMMEDIATE') {
            await this.dispatch(userId, prefs as any, eventType, data, content);
        } else {
    ...
    async dispatch(
        ...
        if (prefs.emailEnabled) {
            await this.sendEmail(userId, content.title, content.body, html, eventType);
        }
    ...
    async enqueueDigest(
        ...
        const item = JSON.stringify({ eventType, title: content.title, body: content.body, ts: Date.now() });
        ...
        this.logger.debug(`Queued ${freq} digest item for user ${userId}: ${eventType}`);
    }
    ...
    async flushDigest(freq: 'HOURLY' | 'DAILY'): Promise<void> {
        ...
        for (const userId of userIds) {
            ...
            const fakeContent = { title: subject, body: bodyText, severity: 'info' as const };

            if ((prefs as any).emailEnabled) {
                await this.sendEmail(userId, subject, bodyText, html, `DIGEST_${freq}`);
            }
            if ((prefs as any).telegramEnabled) {
                await this.sendTelegram(userId, subject, bodyText, `DIGEST_${freq}`);
            }
            if ((prefs as any).discordEnabled) {
                await this.sendDiscord(userId, subject, bodyText, `DIGEST_${freq}`);
            }

            this.logger.log(`Flushed ${freq} digest for user ${userId}: ${parsed.length} items`);
        }
    }

    // ─── Internal send helpers ────────────────────────────────────────────────

    private async sendEmail(userId: string, subject: string, text: string, html: string, eventType: string): Promise<void> {
        ...
        await this.writeHistory(userId, 'EMAIL', eventType, { subject, text }, success, error);
    }

    private async sendTelegram(userId: string, title: string, body: string, eventType: string): Promise<void> {
        ...
        await this.writeHistory(userId, 'TELEGRAM', eventType, { chatId, title, body }, success, error);
    }

    private async sendDiscord(userId: string, title: string, body: string, eventType: string): Promise<void> {
        ...
        await this.writeHistory(userId, 'DISCORD', eventType, { channelId, title, body }, success, error);
    }

    private async pushInApp(userId: string, content: ReturnType<TemplatesService['build']>): Promise<void> {
        try {
    ...
    private async loadPrefs(userId: string): Promise<any | null> {
    ...
    private async writeHistory(
        ...
        try {
            await this.prisma.notificationHistory.create({
                data: { userId, channel, eventType, payload: payload as any, success, error },
            });
        } catch (err: any) {
