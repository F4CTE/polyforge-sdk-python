...
export class SigningService {
    ...
    constructor(
        ...
        this.builderPassphrase = this.config.get<string>('POLY_BUILDER_PASSPHRASE') ?? '';
        this.isDev = this.config.get<string>('NODE_ENV') !== 'production';
    }

    async signOrder(dto: SignOrderDto): Promise<SignedOrder> {
        const { userId, requestId, tokenId, side, size, price, orderType, expiration } = dto;
        ...
        if (this.isDev) {
            order = this.stubSign({ tokenId, side, size, price, orderType, expiration, sigType: creds.sigType });
        } else {
            order = await this.eip712Sign(creds, { tokenId, side, size, price, orderType, expiration });
        }

        const builderHeaders = this.buildBuilderHeaders(requestId);

        this.logger.log(`Order signed for user=${userId} requestId=${requestId} tokenId=${tokenId} side=${side}`);
    ...
    private async eip712Sign(
        creds: { privateKey: string; apiKey: string; apiSecret: string; apiPassphrase: string; safeAddress: string | null; sigType: number },
        params: { tokenId: string; side: 'BUY' | 'SELL'; size: number; price: number; orderType: string; expiration?: number },
    ...
    private buildBuilderHeaders(requestId: string): SignedOrder['builderHeaders'] {
