...
export class CredentialsService {
    ...
    async importCredentials(dto: ImportCredentialsDto): Promise<void> {
        const { userId, privateKey, apiKey, apiSecret, apiPassphrase, safeAddress, sigType } = dto;
        ...
        this.logger.log(`Credentials stored for user ${userId} (sigType=${sigType})`);
    }

    async deleteCredentials(userId: string): Promise<void> {
        const existing = await this.prisma.userCredential.findUnique({ where: { userId } });
        if (!existing) {
    ...
    async getDecryptedCredentials(userId: string): Promise<{
        ...
        const result = {
            privateKey:    this.encryption.decryptField(row.privateKeyCt,    row.privateKeyIv,    row.privateKeyTag,    dek),
            apiKey:        this.encryption.decryptField(row.apiKeyCt,        row.apiKeyIv,        row.apiKeyTag,        dek),
            apiSecret:     this.encryption.decryptField(row.apiSecretCt,     row.apiSecretIv,     row.apiSecretTag,     dek),
            apiPassphrase: this.encryption.decryptField(row.apiPassphraseCt, row.apiPassphraseIv, row.apiPassphraseTag, dek),
