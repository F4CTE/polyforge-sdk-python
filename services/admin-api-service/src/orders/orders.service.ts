...
export class OrdersService {
    ...
    async replayDlqEntry(intentId: string) {
        ...
        if (!entries) {
            throw new NotFoundException({ code: 'NOT_FOUND', message: 'DLQ entry not found' });
        }
        ...
        for (const entry of entries) {
            ...
            const intentData = fieldsMap['intentId'] ?? (fieldsMap['intent'] ? JSON.parse(fieldsMap['intent'])?.intentId : undefined);
