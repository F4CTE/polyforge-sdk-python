...
describe('UsersService', () => {
    ...
    describe('findAll', () => {
        it('returns paginated user list with correct shape', async () => {
            const users = [makeUser(), makeUser({ id: 'user-2', email: 'bob@example.com', username: 'bob' })];
    ...
    describe('findOne', () => {
        it('returns user without sensitive fields', async () => {
            ...
            expect((result as any).totpBackupCodes).toBeUndefined();
        });
    ...
    describe('suspend', () => {
        it('suspends an active user and returns suspension receipt', async () => {
            ...
            prisma.user.findUnique.mockResolvedValue(user as any);
            prisma.user.update.mockResolvedValue({ ...user, suspended: true, suspendedReason: 'Violation' } as any);
        ...
        it('passes reason to update call', async () => {
            const user = makeUser({ suspended: false });
            prisma.user.findUnique.mockResolvedValue(user as any);
            prisma.user.update.mockResolvedValue({ ...user, suspended: true, suspendedReason: 'TOS breach' } as any);

            await service.suspend('user-1', { reason: 'TOS breach' });

            expect(prisma.user.update).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ suspended: true, suspendedReason: 'TOS breach' }),
                }),
            );
        });
        ...
        it('throws CONFLICT/ALREADY_SUSPENDED when user is already suspended', async () => {
            ...
            await expect(service.suspend('user-1', { reason: 'again' })).rejects.toMatchObject({
        ...
        it('throws NotFoundException when user does not exist', async () => {
            ...
            await expect(service.suspend('ghost', { reason: 'test' })).rejects.toThrow(NotFoundException);
        });
        ...
        it('throws NOT_FOUND when user is soft-deleted', async () => {
            ...
            await expect(service.suspend('user-1', { reason: 'test' })).rejects.toMatchObject({
                response: { code: 'NOT_FOUND' },
            });
        });
    });
    ...
    describe('unsuspend', () => {
        it('unsuspends a user and returns { suspended: false }', async () => {
            const user = makeUser({ suspended: true });
            prisma.user.findUnique.mockResolvedValue(user as any);
            prisma.user.update.mockResolvedValue({ ...user, suspended: false, suspendedReason: null } as any);
        ...
        it('throws NotFoundException when user does not exist', async () => {
            ...
            await expect(service.unsuspend('ghost')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('updateLimits', () => {
        it('upserts and returns limits for a valid user', async () => {
            const user = makeUser();
            const limits = { userId: 'user-1', maxStrategies: 5, maxDailyOrders: 100 };
            prisma.user.findUnique.mockResolvedValue(user as any);
    ...
    describe('findUserOrFail (via public API)', () => {
        it('throws NOT_FOUND for nonexistent user id via suspend', async () => {
            ...
            await expect(service.suspend('nonexistent', { reason: 'x' })).rejects.toMatchObject({
                response: { code: 'NOT_FOUND' },
            });
        });

        it('throws NOT_FOUND for soft-deleted user via updateLimits', async () => {
            prisma.user.findUnique.mockResolvedValue(makeUser({ deleted: true }) as any);
