...
describe('AdminsService', () => {
    ...
    describe('findAll', () => {
        it('returns all admins ordered by createdAt asc', async () => {
            const admins = [makeAdmin(), makeAdmin({ id: 'admin-2', email: 'mod@polyforge.com' })];
    ...
    describe('create', () => {
        it('creates a new admin and returns safe admin object', async () => {
            ...
            const created = makeAdmin({ email: 'new@polyforge.com', displayName: 'New Admin', role: 'MODERATOR' });
            adminDb.admin.create.mockResolvedValue(created as any);

            const dto = { email: 'new@polyforge.com', displayName: 'New Admin', password: 'Secret1!', role: 'MODERATOR' as any };
        ...
        it('hashes the password before saving (never stores plaintext)', async () => {
            ...
            const dto = { email: 'new@polyforge.com', displayName: 'Admin', password: 'Secret1!', role: 'MODERATOR' as any };
        ...
        it('sets active:true on creation', async () => {
            ...
            const dto = { email: 'x@x.com', displayName: 'X', password: 'Secret1!', role: 'MODERATOR' as any };
            await service.create(dto);
        ...
        it('throws ConflictException (EMAIL_TAKEN) when email already exists', async () => {
            ...
            const dto = { email: 'admin@polyforge.com', displayName: 'Dup', password: 'Secret1!', role: 'MODERATOR' as any };
    ...
    describe('update', () => {
        it('updates displayName for a valid admin', async () => {
            ...
            const result = await service.update('admin-1', 'admin-999', { displayName: 'Updated Name' });
        ...
        it('allows requester to update their own displayName without triggering SELF_MODIFY', async () => {
            adminDb.admin.findUnique.mockResolvedValue(makeAdmin() as any);
            adminDb.admin.update.mockResolvedValue(makeAdmin({ displayName: 'Me Updated' }) as any);

            const result = await service.update('admin-1', 'admin-1', { displayName: 'Me Updated' });
        ...
        it('invalidates sessions when role changes', async () => {
            adminDb.admin.findUnique.mockResolvedValue(makeAdmin() as any);
            adminDb.admin.update.mockResolvedValue(makeAdmin({ role: 'MODERATOR' }) as any);

            const client = redis.getClient();
            await service.update('admin-1', 'admin-999', { role: 'MODERATOR' as any });
        ...
        it('invalidates sessions when active status changes', async () => {
            adminDb.admin.findUnique.mockResolvedValue(makeAdmin() as any);
            adminDb.admin.update.mockResolvedValue(makeAdmin({ active: false }) as any);
        ...
        it('does NOT invalidate sessions for displayName-only update', async () => {
            ...
            const client = redis.getClient();
            await service.update('admin-1', 'admin-999', { displayName: 'No Session Kill' });
        ...
        it('throws NotFoundException when admin does not exist', async () => {
            ...
            await expect(service.update('ghost', 'admin-999', { displayName: 'x' })).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('deactivate', () => {
        it('deactivates another admin and returns { deactivated: true }', async () => {
            adminDb.admin.findUnique.mockResolvedValue(makeAdmin({ id: 'admin-2' }) as any);
            adminDb.admin.update.mockResolvedValue(makeAdmin({ id: 'admin-2', active: false }) as any);
        ...
        it('throws NotFoundException when admin does not exist', async () => {
            ...
            await expect(service.deactivate('ghost', 'admin-1')).rejects.toThrow(NotFoundException);
        });
    ...
    describe('invalidateAdminSessions (via deactivate)', () => {
        it('deletes keys whose value matches the adminId', async () => {
            const client = {
                scan: vi.fn()
                    .mockResolvedValueOnce(['0', ['admin:session:tok1', 'admin:session:tok2']]),
