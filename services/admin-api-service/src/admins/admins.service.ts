...
export class AdminsService {
    ...
    async create(dto: CreateAdminDto) {
        const existing = await this.adminDb.admin.findUnique({ where: { email: dto.email } });
        if (existing) {
            throw new ConflictException({ code: 'EMAIL_TAKEN', message: 'An admin with this email already exists' });
        }
    ...
    async update(id: string, requesterId: string, dto: UpdateAdminDto) {
        ...
        if (id === requesterId && (dto.role !== undefined || dto.active === false)) {
            throw new ForbiddenException({ code: 'SELF_MODIFY', message: 'You cannot change your own role or deactivate your own account' });
        }
    ...
    async deactivate(id: string, requesterId: string) {
        if (id === requesterId) {
            throw new ForbiddenException({ code: 'SELF_DEACTIVATE', message: 'You cannot deactivate your own account' });
        }
    ...
    private async invalidateAdminSessions(adminId: string): Promise<void> {
        ...
        let cursor = '0';
        do {
            const [nextCursor, keys] = await client.scan(cursor, 'MATCH', 'admin:session:*', 'COUNT', 100);
            cursor = nextCursor;
            if (keys.length > 0) {
