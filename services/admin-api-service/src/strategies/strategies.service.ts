...
const STRATEGY_ENGINE_URL = process.env.STRATEGY_ENGINE_URL ?? 'http://strategy-engine:3006';
    ...
    async forceStop(id: string) {
        ...
        if (strategy.status === 'RUNNING' || strategy.status === 'PAUSED') {
            try {
                ...
                this.logger.warn(`Strategy engine call failed for force-stop ${id}: ${err?.message}`);
            }
        }
