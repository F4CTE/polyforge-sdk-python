...
describe('StrategiesService', () => {
    ...
    describe('forceStop', () => {
        it('calls strategy-engine API when strategy is RUNNING', async () => {
            ...
            prisma.strategy.update.mockResolvedValue({ ...strategy, status: 'IDLE' } as any);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.forceStop('ghost')).rejects.toThrow(NotFoundException);
        });
    ...
    describe('unpublish', () => {
        it('sets visibility to PRIVATE', async () => {
            ...
            prisma.strategy.findUnique.mockResolvedValue(strategy as any);
            prisma.strategy.update.mockResolvedValue({ ...strategy, visibility: 'PRIVATE' } as any);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.unpublish('ghost')).rejects.toThrow(NotFoundException);
        });
