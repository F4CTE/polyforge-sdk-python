...
describe('NotificationsAdminService', () => {
    ...
    describe('broadcast', () => {
        ...
        it('broadcasts to all active verified users when userIds is empty', async () => {
            ...
            const dto = { channel: 'PUSH', templateId: 'tpl-broadcast', subject: 'Announcement' };
            const result = await service.broadcast(dto as any);
        ...
        it('queries only non-deleted, non-suspended, email-verified users for broadcast all', async () => {
            ...
            await service.broadcast({ channel: 'EMAIL', templateId: 'tpl-1', subject: 'x' } as any);
        ...
        it('serializes empty metadata as "{}" when not provided', async () => {
            const dto = { userIds: ['user-1'], channel: 'EMAIL', templateId: 'tpl-x', subject: 'x' };
        ...
        it('returns queued: 0 when broadcast-all finds no eligible users', async () => {
            ...
            const result = await service.broadcast({ channel: 'PUSH', templateId: 'x', subject: 'y' } as any);
