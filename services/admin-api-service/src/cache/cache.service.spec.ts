...
function makeRedisClient(overrides: Record<string, unknown> = {}) {
    return {
        info: vi.fn().mockResolvedValue('used_memory_human:1.50M\r\nused_memory_peak_human:2.00M\r\n'),
...
describe('CacheAdminService', () => {
    ...
    describe('getStats', () => {
        ...
        it('counts keys for each of the 5 known prefixes', async () => {
            ...
            client.scan
                .mockResolvedValue(['0', ['k1', 'k2']]);

            const result = await service.getStats();

            // 5 prefixes × 2 keys each
            const totalPrefixKeys = Object.values(result.keysByPrefix).reduce((a, b) => a + b, 0);
    ...
    describe('flushPattern', () => {
        ...
        it('deletes matching keys and returns the count', async () => {
            client.scan
                .mockResolvedValueOnce(['0', ['cache:price:ETH', 'cache:price:BTC']]);

            const result = await service.flushPattern('cache:price:*');

            expect(result.keysDeleted).toBe(2);
            expect(client.del).toHaveBeenCalledWith('cache:price:ETH', 'cache:price:BTC');
        });
