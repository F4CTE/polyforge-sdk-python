...
export type Scenario = 'normal' | 'volatile' | 'api_down' | 'rate_limited' | 'slow';
...
export class ScenarioService implements OnModuleInit {
    ...
    constructor() {
        ...
        const valid: Scenario[] = ['normal', 'volatile', 'api_down', 'rate_limited', 'slow'];
    ...
    getOrderBook(tokenId: string): { bids: Array<{price: string; size: string}>; asks: Array<{price: string; size: string}> } {
    ...
    private startPriceSimulation() {
        ...
        setInterval(() => {
            this.prices.forEach((live, tokenId) => {
                ...
                this.prices.set(tokenId, { price: newPrice, trend: drift > 0 ? 1 : drift < 0 ? -1 : 0 });

                // Update complementary NO token
                const noTokenId = tokenId.replace('-yes', '-no');
                if (this.prices.has(noTokenId)) {
                    this.prices.set(noTokenId, { price: 1 - newPrice, trend: -live.trend as -1 | 0 | 1 });
                }
            });
        }, intervalMs);
    }
