...
export class WsFeedService implements OnModuleInit, OnModuleDestroy {
    ...
    onModuleInit() {
        ...
        this.logger.log(`WebSocket feed listening on port ${WS_PORT} [scenario: ${this.scenario.scenario}]`);
    }
    ...
    private startBroadcasts() {
        this.priceTimer = setInterval(() => this.broadcastPrices(), PRICE_INTERVAL_MS);
    ...
    broadcastOrderEvent(event: {
        type: 'ORDER_PLACED' | 'ORDER_SUBMITTED' | 'ORDER_FILLED' | 'ORDER_PARTIAL' | 'ORDER_CANCELLED' | 'ORDER_FAILED';
        orderId: string;
        [key: string]: any;
    }) {
        if (!this.wss) return;
    ...
    private sendPriceSnapshot(ws: WebSocket, tokenIds: string[]) {
        ...
        ws.send(JSON.stringify({ type: 'PRICE_SNAPSHOT', prices, timestamp: Date.now() }));
    }
