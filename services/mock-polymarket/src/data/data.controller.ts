...
export class DataController {
    ...
    async getPositions(
        ...
        reply.send({ walletAddress, positions: [], updatedAt: new Date().toISOString() });
    }
    ...
    private generatePriceHistory(currentPrice: number, interval: string): Array<{ t: number; p: string }> {
        const now = Date.now();
        const points = 50;
        ...
        switch (interval) {
            ...
            default:    intervalMs = 3_600_000; break;  // 1h
        }
        ...
        for (let i = points; i >= 0; i--) {
            ...
            price = Math.max(0.02, Math.min(0.98, price + (Math.random() - 0.5) * 0.01));
        }

        return result;
    }

    private generateRecentTrades(tokenId: string, count: number): Array<{ price: string; size: string; side: 'buy'|'sell'; timestamp: number }> {
        const now = Date.now();
        const currentPrice = this.scenario.getPrice(tokenId);

        return Array.from({ length: count }, (_, i) => ({
            price: Math.max(0.01, currentPrice + (Math.random() - 0.5) * 0.04).toFixed(2),
