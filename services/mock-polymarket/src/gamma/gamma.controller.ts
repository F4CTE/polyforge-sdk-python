...
import { FIXTURE_MARKETS, MARKETS_BY_ID, MARKETS_BY_SLUG } from '../fixtures/markets';
...
export class GammaController {
    ...
    private async guard(reply: FastifyReply, ip: string): Promise<boolean> {
        if (this.scenario.shouldReturnDown()) {
            reply.status(503).send({ error: 'Service Unavailable', code: 'SERVICE_DOWN' });
            return false;
        }
    ...
    async listMarkets(
        ...
        if (category) markets = markets.filter(m => m.category.toLowerCase() === category.toLowerCase());
        if (closed !== undefined) markets = markets.filter(m => m.closed === (closed === 'true'));
    ...
    async listEvents(
        ...
        const series = FIXTURE_MARKETS
            .filter(m => m.seriesSlug)
            .reduce((acc, m) => {
