...
describe('WaitlistService', () => {
    ...
    beforeEach(() => {
        ...
        mail  = { sendWaitlistConfirmationEmail: vi.fn().mockResolvedValue(undefined) };
    ...
    describe('join', () => {
        ...
        it('sends a confirmation email on first signup (zadd === 1)', async () => {
            ...
            await vi.waitFor(() => {
                expect(mail.sendWaitlistConfirmationEmail).toHaveBeenCalledWith('new@example.com');
            });
        });
        ...
        it('normalises email to lowercase before storing', async () => {
            ...
            expect(redisClient.zadd).toHaveBeenCalledWith(
                'waitlist:emails', 'NX', expect.any(Number), 'upper@example.com',
            );
        });

        it('does not throw when confirmation email fails (fire-and-forget)', async () => {
            redisClient.zadd.mockResolvedValue(1);
            mail.sendWaitlistConfirmationEmail.mockRejectedValue(new Error('SMTP down'));
            await expect(service.join({ email: 'x@y.com' })).resolves.toEqual({ joined: true });
        });
    });
    ...
    describe('list', () => {
        ...
        it('maps raw redis pairs to { email, joinedAt } objects', async () => {
            ...
            redisClient.zrange.mockResolvedValue(['a@b.com', String(ts), 'c@d.com', String(ts + 1000)]);
            const result = await service.list();
            expect(result).toHaveLength(2);
            expect(result[0]).toEqual({ email: 'a@b.com', joinedAt: new Date(ts).toISOString() });
            expect(result[1]).toEqual({ email: 'c@d.com', joinedAt: new Date(ts + 1000).toISOString() });
        });
    });
    ...
    describe('remove', () => {
        it('calls zrem with the correct key and lowercased email', async () => {
            ...
            expect(redisClient.zrem).toHaveBeenCalledWith('waitlist:emails', 'test@example.com');
        });
    });
