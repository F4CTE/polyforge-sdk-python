...
async function generate() {
    ...
    const config = new DocumentBuilder()
        .setTitle('Polyforge Auth API')
        .setDescription('User authentication, registration, email verification, and password management')
        .setVersion('1.0')
    ...
    await writeFile(join(outDir, 'swagger.auth.json'), JSON.stringify(document, null, 2));

    console.log('✓ dist/swagger.auth.json written');
