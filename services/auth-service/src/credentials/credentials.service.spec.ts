...
const verifiedUser = () => userFactory({ emailVerified: true, polymarketConnected: false });
const connectedUser = () => userFactory({ emailVerified: true, polymarketConnected: true });
...
describe('CredentialsService', () => {
    ...
    describe('import', () => {
        ...
        it('calls signer-service and marks user as connected on success', async () => {
            ...
            db.user.findUniqueOrThrow.mockResolvedValue(user as any);
            db.user.update.mockResolvedValue({ ...user, polymarketConnected: true } as any);
        ...
        it('throws SIGNER_ERROR (502) when signer-service returns non-OK', async () => {
            const user = verifiedUser();
            db.user.findUniqueOrThrow.mockResolvedValue(user as any);
            fetchSpy.mockResolvedValue({ ok: false, status: 500, json: vi.fn().mockResolvedValue({}) });
    ...
    describe('delete', () => {
        ...
        it('calls signer-service DELETE and disconnects user on success', async () => {
            ...
            db.user.findUniqueOrThrow.mockResolvedValue(user as any);
            db.user.update.mockResolvedValue({ ...user, polymarketConnected: false } as any);
            fetchSpy.mockResolvedValue({ ok: true });
