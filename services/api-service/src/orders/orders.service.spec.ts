...
describe('OrdersService', () => {
    ...
    describe('list', () => {
        ...
        it('adds status filter when provided', async () => {
            ...
            await service.list('user-uuid-1', makeOrderQuery({ status: 'FILLED' }) as any);
        ...
        it('adds strategyId filter when provided', async () => {
            ...
            await service.list('user-uuid-1', makeOrderQuery({ strategyId: 'strategy-abc' }) as any);
        ...
        it('adds createdAt.gte filter when from is provided', async () => {
            ...
            await service.list('user-uuid-1', makeOrderQuery({ from: '2025-01-01T00:00:00.000Z' }) as any);
        ...
        it('adds createdAt.lte filter when to is provided', async () => {
            ...
            await service.list('user-uuid-1', makeOrderQuery({ to: '2025-12-31T23:59:59.000Z' }) as any);
        ...
        it('calculates correct skip for page 2 limit 10', async () => {
            ...
            await service.list('user-uuid-1', makeOrderQuery({ page: 2, limit: 10 }) as any);
    ...
    describe('closePosition', () => {
        it('publishes to Redis stream and creates a PENDING order', async () => {
            db.user.findUnique.mockResolvedValue(makeUser({ polymarketConnected: true }) as any);
            db.position.findFirst.mockResolvedValue(makePosition() as any);
            db.order.create.mockResolvedValue(makeOrder() as any);

            const result = await service.closePosition('user-uuid-1', makeClosePositionDto() as any);
        ...
        it('uses dto.size when explicitly provided', async () => {
            ...
            await service.closePosition('user-uuid-1', makeClosePositionDto({ size: '25.00' }) as any);
        ...
        it('generates a unique intentId for each call', async () => {
            ...
            const result1 = await service.closePosition('user-uuid-1', makeClosePositionDto() as any);

            db.order.create.mockResolvedValue(makeOrder({ id: 'order-uuid-2', intentId: 'intent-uuid-2' }) as any);

            const result2 = await service.closePosition('user-uuid-1', makeClosePositionDto() as any);
