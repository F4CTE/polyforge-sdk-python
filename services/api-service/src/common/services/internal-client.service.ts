...
export class InternalClientService {
    ...
    constructor(
        ...
        this.secret = this.config.get<string>('INTERNAL_JWT_SECRET', 'dev-internal-jwt-secret');
    }
    ...
    async post(baseUrl: string, audience: string, path: string): Promise<Response> {
    ...
    async delete(baseUrl: string, audience: string, path: string): Promise<Response> {
        const token = this.issueToken(audience);
        const url = `${baseUrl}${path}`;
        const res = await fetch(url, {
            method: 'DELETE',
    ...
    async get(baseUrl: string, audience: string, path: string): Promise<Response> {
        const token = this.issueToken(audience);
        const url = `${baseUrl}${path}`;
        const res = await fetch(url, {
            headers: { Authorization: `Bearer ${token}` },
