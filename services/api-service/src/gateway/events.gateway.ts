...
export class EventsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    private handleMessage(client: AuthedSocket, msg: any) {
        ...
        if (!client.isAuthenticated) {
            if (type !== 'AUTH') {
                this.send(client, { type: 'AUTH_ERROR', message: 'Must authenticate first' });
                client.terminate();
                return;
            }
    ...
    pushStrategyEvent(strategyId: string, userId: string, type: string, payload: Record<string, any>) {
