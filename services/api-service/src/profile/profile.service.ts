...
export class ProfileService {
    ...
    async getProfile(username: string, viewerUserId?: string): Promise<any> {
        ...
        const [followersCount, followingCount, publicStrategyCount] = await Promise.all([
            ...
            this.prisma.strategy.count({
                where: { userId: user.id, visibility: 'PUBLIC' as any, status: { not: StrategyStatus.ARCHIVED } },
            }),
        ]);
    ...
    async toggleFollow(username: string, viewerUserId: string): Promise<any> {
        ...
        const existing = await this.prisma.follow.findUnique({
            where: { followerId_followingId: { followerId: viewerUserId, followingId: target.id } },
        });
        ...
        const followersCount = await this.prisma.follow.count({ where: { followingId: target.id } });
