...
export class DiscoverService {
    ...
    async discover(userId: string, query: DiscoverQueryDto): Promise<PaginatedResponse<any>> {
        ...
        const orderBy = query.sort === 'newest' ? { createdAt: 'desc' as const }
            : query.sort === 'top_pnl' ? { likeCount: 'desc' as const }  // approximate
            : query.sort === 'most_forked' ? { forkCount: 'desc' as const }
    ...
    async leaderboard(query: LeaderboardQueryDto): Promise<PaginatedResponse<any>> {
        const page = query.page ?? 1;
        ...
        const since = period === '7d' ? new Date(Date.now() - 7 * 86400_000)
            : period === '30d' ? new Date(Date.now() - 30 * 86400_000)
            : new Date(0); // allTime
