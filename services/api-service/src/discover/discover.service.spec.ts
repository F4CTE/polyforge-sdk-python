...
describe('DiscoverService', () => {
    ...
    describe('discover', () => {
        ...
        it('strips triggers/conditions/actions/safety from UNLISTED strategies', async () => {
            ...
            const returned = result.data[0] as any;
    ...
    describe('leaderboard', () => {
        it('returns a paginated leaderboard with user data', async () => {
            ...
            const users = [
                { id: 'user-uuid-1', username: 'alice', displayName: 'Alice', avatarUrl: null },
                { id: 'user-uuid-2', username: 'bob', displayName: 'Bob', avatarUrl: null },
            ];
            db.$queryRaw
        ...
        it('caps limit at 100', async () => {
            ...
            await expect(service.leaderboard(makeLeaderboardQuery({ limit: 200 }))).resolves.toBeDefined();
        });

        it('fills unknown users with empty string fallbacks', async () => {
            const rows = [{ userId: 'user-uuid-orphan', pnl: '100', tradeCount: '1' }];
            db.$queryRaw
                .mockResolvedValueOnce(rows as any)
        ...
        it('handles null pnl in rows gracefully', async () => {
            ...
            const users = [{ id: 'user-uuid-1', username: 'alice', displayName: 'Alice', avatarUrl: null }];
            db.$queryRaw
                .mockResolvedValueOnce(rows as any)
                .mockResolvedValueOnce([{ cnt: '1' }] as any);
            db.user.findMany.mockResolvedValue(users as any);
