...
let DiscoverService = class DiscoverService {
    ...
    async discover(userId, query) {
        ...
        const orderBy = query.sort === "newest"
            ? { createdAt: "desc" }
            : query.sort === "top_pnl"
                ? { likeCount: "desc" } // approximate
                : query.sort === "most_forked"
                    ? { forkCount: "desc" }
                    : { likeCount: "desc" }; // popular default
        ...
        return (0, pagination_dto_1.paginate)(result, total, page, limit);
    }
    async leaderboard(query) {
        ...
        const result = (0, pagination_dto_1.paginate)(data, total, page, limit);
