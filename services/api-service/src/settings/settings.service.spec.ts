...
describe('SettingsService', () => {
    ...
    describe('updateProfile', () => {
        it('updates and returns the user profile', async () => {
            ...
            const result = await service.updateProfile('user-uuid-1', makeUpdateProfileDto() as any);
        ...
        it('includes twitterHandle in data when provided', async () => {
            ...
            await service.updateProfile('user-uuid-1', { twitterHandle: '@alice' } as any);
    ...
    describe('updateNotifications', () => {
        it('upserts notification preferences and returns the record', async () => {
            const prefs = { userId: 'user-uuid-1', emailOnFill: true, emailOnAlert: false };
            db.notificationPreference.upsert.mockResolvedValue(prefs as any);

            const result = await service.updateNotifications('user-uuid-1', { emailOnFill: true, emailOnAlert: false });
        ...
        it('handles an empty dto without throwing', async () => {
            ...
            await expect(service.updateNotifications('user-uuid-1', {})).resolves.toBeDefined();
        });
    });
    ...
    describe('updatePassword', () => {
        it('updates the password and returns a success message', async () => {
            ...
            db.user.findUniqueOrThrow.mockResolvedValue({ passwordHash: hash } as any);
            db.user.update.mockResolvedValue({} as any);

            const result = await service.updatePassword('user-uuid-1', makeUpdatePasswordDto() as any);
        ...
        it('calls prisma.user.update with a bcrypt hash of the new password', async () => {
            ...
            await service.updatePassword('user-uuid-1', makeUpdatePasswordDto() as any);
        ...
        it('throws INVALID_CREDENTIALS (401) when current password is wrong', async () => {
            ...
            await expect(
                service.updatePassword('user-uuid-1', makeUpdatePasswordDto({ currentPassword: 'WrongPassword1!' }) as any),
