...
const INTERNAL_SERVICES: { name: string; label: string; url: string }[] = [
  { name: "auth",         label: "Auth Service",          url: "http://auth-service:3001/health" },
  { name: "strategy",     label: "Strategy Engine",       url: "http://strategy-engine:3006/health" },
  { name: "order",        label: "Order Service",         url: "http://order-service:3007/health" },
  { name: "paper",        label: "Paper Order Service",   url: "http://paper-order-service:3008/health" },
  { name: "backtest",     label: "Backtest Service",      url: "http://backtest-service:3009/health" },
  { name: "notification", label: "Notification Service",  url: "http://notification-service:3010/health" },
  { name: "marketdata",   label: "Market Data Service",   url: "http://market-data-service:3005/health" },
  { name: "signer",       label: "Signer Service",        url: "http://signer-service:3012/health" },
  { name: "bot",          label: "Bot Service",           url: "http://bot-service:3011/health" },
];

async function pingService(url: string): Promise<{ ok: boolean; latencyMs: number }> {
...
export class StatusController {
  ...
  async status() {
    ...
    const [dbResult, redisResult] = await Promise.allSettled([
      this.prisma.$queryRaw<[{ one: number }]>`SELECT 1 AS one`.then(() => true),
      this.redis.getClient().ping().then(() => true),
    ]);
    ...
    return {
      ...
      services: [
        ...
        ...svcResults.map((s) => ({ name: s.name, label: s.label, ok: s.ok, latencyMs: s.latencyMs })),
      ],
    };
  }
}
