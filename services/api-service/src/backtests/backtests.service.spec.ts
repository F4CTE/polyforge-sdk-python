...
describe('BacktestsService', () => {
    ...
    describe('list', () => {
        ...
        it('passes correct skip/take to findMany', async () => {
            ...
            await service.list('user-uuid-1', makeQuery({ page: 3, limit: 10 }) as any);
        ...
        it('filters by strategyId when provided', async () => {
            ...
            await service.list('user-uuid-1', makeQuery({ strategyId: 'strategy-abc' }) as any);
        ...
        it('filters by status when provided', async () => {
            ...
            await service.list('user-uuid-1', makeQuery({ status: 'COMPLETED' }) as any);
        ...
        it('calculates totalPages and hasNext correctly', async () => {
            ...
            const result = await service.list('user-uuid-1', makeQuery({ page: 1, limit: 20 }) as any);
    ...
    describe('create', () => {
        it('returns a stub result immediately when quickMode is true', async () => {
            const result = await service.create('user-uuid-1', makeCreateDto({ quickMode: true }) as any);
        ...
        it('does NOT persist or publish to Redis in quickMode', async () => {
            await service.create('user-uuid-1', makeCreateDto({ quickMode: true }) as any);
        ...
        it('creates a QUEUED run and publishes to Redis stream when not quickMode', async () => {
            ...
            const result = await service.create('user-uuid-1', makeCreateDto() as any);
        ...
        it('creates the run with status QUEUED', async () => {
            ...
            expect(db.backtestRun.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ status: 'QUEUED', userId: 'user-uuid-1' }),
                }),
            );
        });
        ...
        it('defaults strategyId to empty string when not provided', async () => {
            ...
            await service.create('user-uuid-1', makeCreateDto({ strategyId: undefined }) as any);
        ...
        it('parses dateRangeStart and dateRangeEnd when provided', async () => {
            ...
            expect(dataArg.dateRangeStart).toEqual(new Date('2024-03-01T00:00:00.000Z'));
            expect(dataArg.dateRangeEnd).toEqual(new Date('2024-09-30T00:00:00.000Z'));
        });
        ...
        it('uses epoch start and current date when dateRange not provided', async () => {
            ...
            await service.create('user-uuid-1', makeCreateDto({ dateRangeStart: undefined, dateRangeEnd: undefined }) as any);
        ...
        it('includes a ts field in the Redis stream payload', async () => {
            ...
            const streamPayload = (redis.xadd as ReturnType<typeof vi.fn>).mock.calls[0][1];
    ...
    describe('findOne', () => {
        ...
        it('throws NotFoundException (404) when the run does not exist', async () => {
            ...
            await expect(service.findOne('nonexistent-id', 'user-uuid-1')).rejects.toThrow(NotFoundException);
        });

        it('throws NOT_FOUND error code when the run does not exist', async () => {
            db.backtestRun.findUnique.mockResolvedValue(null);

            await expect(service.findOne('nonexistent-id', 'user-uuid-1')).rejects.toMatchObject({
        ...
        it('throws NotFoundException (404) when the run belongs to a different user', async () => {
            ...
            await expect(service.findOne('run-uuid-1', 'user-uuid-1')).rejects.toThrow(NotFoundException);
        });
        ...
        it('looks up the run by id', async () => {
            ...
            expect(db.backtestRun.findUnique).toHaveBeenCalledWith({ where: { id: 'run-uuid-1' } });
        });
    });
});
