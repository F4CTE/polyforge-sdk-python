...
describe('PaperService', () => {
    ...
    describe('getSummary', () => {
        it('returns order count, positions array and total pnl', async () => {
            const positions = [makePosition(), makePosition({ tokenId: 'token-uuid-2', realizedPnl: '5.50' })];
        ...
        it('maps position fields correctly', async () => {
            const position = makePosition({ tokenId: 'token-uuid-1', outcome: 'NO', size: '50.00' });
        ...
        it('queries paperOrder.count and paperPosition.findMany with the given userId', async () => {
            ...
            expect(db.paperOrder.count).toHaveBeenCalledWith({ where: { userId: 'user-uuid-42' } });
    ...
    describe('reset', () => {
        ...
        it('calls deleteMany on both paperOrder and paperPosition with the userId', async () => {
            ...
            expect(db.paperOrder.deleteMany).toHaveBeenCalledWith({ where: { userId: 'user-uuid-1' } });
            expect(db.paperPosition.deleteMany).toHaveBeenCalledWith({ where: { userId: 'user-uuid-1' } });
        });
