...
function makeQuery(overrides: Partial<StrategyQueryDto> = {}): StrategyQueryDto {
    return { page: 1, limit: 20, sort: 'createdAt', ...overrides } as StrategyQueryDto;
}

function makePaginationDto(overrides: Partial<PaginationDto> = {}): PaginationDto {
...
describe('StrategiesService', () => {
    ...
    describe('create', () => {
        ...
        it('throws STRATEGY_LIMIT_REACHED when user has 50 strategies', async () => {
            ...
            await expect(service.create('user-1', { name: 'Over limit' } as any)).rejects.toMatchObject({
        ...
        it('throws STRATEGY_LIMIT_REACHED at exactly 50 (boundary)', async () => {
            ...
            await expect(service.create('user-1', { name: 'Limit' } as any))
        ...
        it('allows creation when count is 49', async () => {
            ...
            await expect(service.create('user-1', { name: 'Under limit' } as any)).resolves.toBeDefined();
        });
    });
    ...
    describe('findOne', () => {
        ...
        it('throws NotFoundException when strategy is ARCHIVED', async () => {
            ...
            await expect(service.findOne(strategy.id, 'user-1')).rejects.toMatchObject({
                response: { code: 'NOT_FOUND' },
                status: 404,
            });
        });
        ...
        it('throws ForbiddenException when PRIVATE strategy is accessed by non-owner', async () => {
            const strategy = makeStrategy({ userId: 'owner-id', visibility: 'PRIVATE' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            await expect(service.findOne(strategy.id, 'other-user')).rejects.toMatchObject({
        ...
        it('allows owner to access their own PRIVATE strategy', async () => {
            const strategy = makeStrategy({ userId: 'user-1', visibility: 'PRIVATE' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            await expect(service.findOne(strategy.id, 'user-1')).resolves.toEqual(strategy);
        });

        it('allows any user to view a PUBLIC strategy', async () => {
            const strategy = makeStrategy({ userId: 'owner-id', visibility: 'PUBLIC' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            await expect(service.findOne(strategy.id, 'any-user')).resolves.toEqual(strategy);
        });

        it('allows any user to view an UNLISTED strategy', async () => {
            const strategy = makeStrategy({ userId: 'owner-id', visibility: 'UNLISTED' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            await expect(service.findOne(strategy.id, 'random-user')).resolves.toEqual(strategy);
        });
    });
    ...
    describe('update', () => {
        it('updates and returns the strategy', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            const updated = { ...strategy, name: 'Updated Name', version: 2 };
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.strategy.update.mockResolvedValue(updated as any);

            const result = await service.update(strategy.id, 'user-1', { name: 'Updated Name' } as any);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.update('missing', 'user-1', {} as any)).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when user does not own the strategy', async () => {
            ...
            await expect(service.update(strategy.id, 'user-1', {} as any)).rejects.toMatchObject({
                response: { code: 'FORBIDDEN' },
                status: 403,
            });
        });
        ...
        it('throws STRATEGY_IS_RUNNING when editing blocks on a running strategy', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.RUNNING });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            const dto: UpdateStrategyDto = { triggers: [{ type: 'PRICE', config: {} }] } as any;

            await expect(service.update(strategy.id, 'user-1', dto)).rejects.toMatchObject({
    ...
    describe('remove', () => {
        it('soft-deletes by setting status to ARCHIVED', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.strategy.update.mockResolvedValue({ ...strategy, status: StrategyStatus.ARCHIVED } as any);
    ...
    describe('start', () => {
        it('calls engine and returns RUNNING status for paper mode', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.strategy.update.mockResolvedValue({ ...strategy, status: StrategyStatus.PAPER } as any);
            vi.mocked(client.post).mockResolvedValue(mockEngineResponse(true, 200));

            const result = await service.start(strategy.id, 'user-1', { mode: 'paper' } as StartStrategyDto);

            expect(result.status).toBe('RUNNING');
            expect(result.startedAt).toBeDefined();
        });

        it('sets PAPER status in DB before engine call for paper mode', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.strategy.update.mockResolvedValue({ ...strategy, status: StrategyStatus.PAPER } as any);
            vi.mocked(client.post).mockResolvedValue(mockEngineResponse(true, 200));

            await service.start(strategy.id, 'user-1', { mode: 'paper' } as StartStrategyDto);
        ...
        it('calls engine and returns RUNNING status for live mode when polymarketConnected', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.user.findUnique.mockResolvedValue({ polymarketConnected: true } as any);
            vi.mocked(client.post).mockResolvedValue(mockEngineResponse(true, 200));

            const result = await service.start(strategy.id, 'user-1', { mode: 'live' } as StartStrategyDto);

            expect(result.status).toBe('RUNNING');
        });

        it('does NOT set PAPER status for live mode', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.user.findUnique.mockResolvedValue({ polymarketConnected: true } as any);
            vi.mocked(client.post).mockResolvedValue(mockEngineResponse(true, 200));

            await service.start(strategy.id, 'user-1', { mode: 'live' } as StartStrategyDto);

            // update should NOT have been called (no PAPER status set)
        ...
        it('throws ALREADY_RUNNING when strategy is already running', async () => {
            ...
            await expect(
                service.start(strategy.id, 'user-1', { mode: 'paper' } as StartStrategyDto),
        ...
        it('throws NOT_CONNECTED for live mode when polymarketConnected is false', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            db.user.findUnique.mockResolvedValue({ polymarketConnected: false } as any);

            await expect(
                service.start(strategy.id, 'user-1', { mode: 'live' } as StartStrategyDto),
            ).rejects.toMatchObject({
        ...
        it('throws ENGINE_ERROR when engine returns non-ok and non-204', async () => {
            ...
            vi.mocked(client.post).mockResolvedValue(
                mockEngineResponse(false, 500, { code: 'ENGINE_ERROR', message: 'Internal error' }),
            );
        ...
        it('uses code from engine error body when available', async () => {
            ...
            db.strategy.update.mockResolvedValue(strategy as any);
            vi.mocked(client.post).mockResolvedValue(
                mockEngineResponse(false, 503, { code: 'STRATEGY_TIMEOUT', message: 'timeout' }),
            );
        ...
        it('throws NotFoundException when strategy not found', async () => {
            ...
            await expect(
                service.start('bad-id', 'user-1', { mode: 'paper' } as StartStrategyDto),
            ).rejects.toBeInstanceOf(NotFoundException);
        });

        it('throws ForbiddenException when user does not own the strategy', async () => {
            const strategy = makeStrategy({ userId: 'other-user', status: StrategyStatus.IDLE });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
    ...
    describe('stop', () => {
        it('calls engine DELETE and sets strategy to IDLE', async () => {
            ...
            db.strategy.update.mockResolvedValue({ ...strategy, status: StrategyStatus.IDLE } as any);
        ...
        it('still sets IDLE when engine returns non-ok (graceful degradation)', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.RUNNING });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            vi.mocked(client.delete).mockResolvedValue(mockEngineResponse(false, 503));
            db.strategy.update.mockResolvedValue({ ...strategy, status: StrategyStatus.IDLE } as any);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.stop('bad-id', 'user-1')).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when user does not own the strategy', async () => {
            ...
            await expect(service.stop(strategy.id, 'user-1')).rejects.toBeInstanceOf(ForbiddenException);
        });
    });
    ...
    describe('pause', () => {
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.pause('bad-id', 'user-1')).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when user does not own the strategy', async () => {
            ...
            await expect(service.pause(strategy.id, 'user-1')).rejects.toBeInstanceOf(ForbiddenException);
        });
    });
    ...
    describe('resume', () => {
        it('calls engine resume endpoint and returns RUNNING status', async () => {
            const strategy = makeStrategy({ userId: 'user-1', status: StrategyStatus.PAUSED });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
            vi.mocked(client.post).mockResolvedValue(mockEngineResponse(true, 200));
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(service.resume('bad-id', 'user-1')).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when user does not own the strategy', async () => {
            ...
            await expect(service.resume(strategy.id, 'user-1')).rejects.toBeInstanceOf(ForbiddenException);
        });
    });
    ...
    describe('fork', () => {
        ...
        it('sets forked strategy to IDLE with version 1 and template false', async () => {
            const original = makeStrategy({ id: 'orig-1', userId: 'owner-id', visibility: 'PUBLIC' });
            db.strategy.findUnique.mockResolvedValue(original as any);
            db.strategy.count.mockResolvedValue(0);
            db.strategy.create.mockResolvedValue(makeStrategy() as any);
        ...
        it('prefixes forked name with "Fork of "', async () => {
            const original = makeStrategy({ id: 'orig-1', name: 'Great Strategy', visibility: 'PUBLIC' });
            db.strategy.findUnique.mockResolvedValue(original as any);
            db.strategy.count.mockResolvedValue(0);
            db.strategy.create.mockResolvedValue(makeStrategy() as any);
        ...
        it('throws NotFoundException when strategy is ARCHIVED', async () => {
            ...
            await expect(service.fork(strategy.id, 'user-1')).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when forking a PRIVATE strategy not owned by user', async () => {
            ...
            await expect(service.fork(strategy.id, 'other-user')).rejects.toMatchObject({
                response: { code: 'FORBIDDEN' },
                status: 403,
            });
        });
    ...
    describe('like', () => {
        it('likes a strategy and returns liked=true with incremented count', async () => {
            ...
            db.strategy.update.mockResolvedValue({ ...strategy, likeCount: 6 } as any);
        ...
        it('unlikes a strategy and returns liked=false with decremented count', async () => {
            ...
            db.strategyLike.findUnique.mockResolvedValue({ userId: 'user-1', strategyId: strategy.id } as any);
            db.strategyLike.delete.mockResolvedValue({} as any);
            db.strategy.update.mockResolvedValue({ ...strategy, likeCount: 4 } as any);
        ...
        it('calls strategyLike.delete when toggling off', async () => {
            ...
            expect(db.strategyLike.delete).toHaveBeenCalledWith({
                where: { userId_strategyId: { userId: 'user-1', strategyId: strategy.id } },
            });
        });
    ...
    describe('listComments', () => {
        ...
        it('queries only non-deleted comments', async () => {
            ...
            const whereArg = (db.strategyComment.findMany as any).mock.calls[0][0].where;
        ...
        it('includes user data in response', async () => {
            ...
            const includeArg = (db.strategyComment.findMany as any).mock.calls[0][0].include;
    ...
    describe('addComment', () => {
        it('creates and returns a new comment', async () => {
            ...
            const result = await service.addComment(strategy.id, 'user-1', { content: 'Great strategy!' } as CreateCommentDto);
        ...
        it('creates comment with correct data', async () => {
            ...
            await service.addComment(strategy.id, 'user-1', { content: 'Hello' } as CreateCommentDto);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(
                service.addComment('bad-id', 'user-1', { content: 'x' } as CreateCommentDto),
            ).rejects.toBeInstanceOf(NotFoundException);
        });

        it('throws ForbiddenException when commenting on a PRIVATE strategy not owned', async () => {
            const strategy = makeStrategy({ userId: 'other-user', visibility: 'PRIVATE' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);

            await expect(
                service.addComment(strategy.id, 'user-1', { content: 'x' } as CreateCommentDto),
            ).rejects.toBeInstanceOf(ForbiddenException);
        });
    });
    ...
    describe('deleteComment', () => {
        it('soft-deletes the comment by setting deleted=true', async () => {
            ...
            db.strategyComment.update.mockResolvedValue({ ...comment, deleted: true } as any);
        ...
        it('throws NotFoundException when comment does not exist', async () => {
            ...
            await expect(service.deleteComment('strat-1', 'bad-id', 'user-1')).rejects.toMatchObject({
                response: { code: 'NOT_FOUND' },
                status: 404,
            });
        });
        ...
        it('throws NotFoundException when comment belongs to a different strategy', async () => {
            const comment = makeComment({ strategyId: 'other-strat', userId: 'user-1' });
            db.strategyComment.findUnique.mockResolvedValue(comment as any);

            await expect(service.deleteComment('strat-1', comment.id, 'user-1')).rejects.toMatchObject({
                response: { code: 'NOT_FOUND' },
                status: 404,
            });
        });
        ...
        it('throws ForbiddenException when user is not the comment author', async () => {
            const comment = makeComment({ strategyId: 'strat-1', userId: 'comment-owner' });
            db.strategyComment.findUnique.mockResolvedValue(comment as any);

            await expect(service.deleteComment('strat-1', comment.id, 'other-user')).rejects.toMatchObject({
                response: { code: 'FORBIDDEN' },
                status: 403,
            });
        });
    ...
    describe('report', () => {
        it('creates a report and returns reportId', async () => {
            ...
            const result = await service.report(strategy.id, 'user-1', { reason: 'SPAM' } as ReportStrategyDto);

            expect(result.reportId).toBe('report-abc');
        });

        it('creates report with correct fields', async () => {
            const strategy = makeStrategy({ id: 'strat-1', userId: 'owner', visibility: 'PUBLIC' });
            db.strategy.findUnique.mockResolvedValue(strategy as any);
        ...
        it('throws NotFoundException when strategy does not exist', async () => {
            ...
            await expect(
                service.report('bad-id', 'user-1', { reason: 'SPAM' } as ReportStrategyDto),
            ).rejects.toBeInstanceOf(NotFoundException);
        });
        ...
        it('throws ForbiddenException when reporting a PRIVATE strategy not visible to reporter', async () => {
            ...
            await expect(
                service.report(strategy.id, 'other-user', { reason: 'SPAM' } as ReportStrategyDto),
            ).rejects.toBeInstanceOf(ForbiddenException);
        });
        ...
        it('allows reporting with no description (optional field)', async () => {
            ...
            await expect(
                service.report(strategy.id, 'user-1', { reason: 'OTHER' } as ReportStrategyDto),
            ).resolves.toBeDefined();
        });
    });
    ...
    describe('listTemplates', () => {
        ...
        it('sets hasNext correctly when more templates exist', async () => {
            db.strategy.findMany.mockResolvedValue(Array.from({ length: 10 }, () => makeStrategy({ template: true })) as any);
