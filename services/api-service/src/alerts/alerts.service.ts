...
export class AlertsService {
    ...
    async create(userId: string, dto: CreateAlertDto): Promise<any> {
        const count = await this.prisma.priceAlert.count({ where: { userId, triggered: false } });
        if (count >= MAX_ALERTS) {
            throw new UnprocessableEntityException({ code: 'ALERT_LIMIT_REACHED', message: 'Maximum 50 alerts allowed' });
        }
    ...
    async remove(id: string, userId: string): Promise<void> {
        ...
        if (!alert) throw new NotFoundException({ code: 'NOT_FOUND', message: 'Alert not found' });
        if (alert.userId !== userId) throw new ForbiddenException({ code: 'FORBIDDEN', message: 'Access denied' });
