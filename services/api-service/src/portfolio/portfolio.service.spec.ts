...
describe('PortfolioService', () => {
    ...
    describe('getPortfolio', () => {
        ...
        it('enriches each position with current price from Redis cache', async () => {
            const position = makePosition({ tokenId: 'token-uuid-1', avgPrice: '0.50', size: '100.00' });
            db.position.findMany.mockResolvedValue([position] as any);
            (redis.get as ReturnType<typeof vi.fn>).mockResolvedValue(JSON.stringify({ price: '0.70' }));
        ...
        it('calculates unrealizedPnl as (currentPrice - avgEntry) * size', async () => {
            const position = makePosition({ avgPrice: '0.50', size: '200.00', tokenId: 'token-uuid-1' });
            db.position.findMany.mockResolvedValue([position] as any);
            (redis.get as ReturnType<typeof vi.fn>).mockResolvedValue(JSON.stringify({ price: '0.80' }));
        ...
        it('accumulates totalUnrealizedPnl across all positions', async () => {
            const positions = [
                makePosition({ id: 'p1', tokenId: 'token-1', avgPrice: '0.50', size: '100.00', realizedPnl: '0' }),
                makePosition({ id: 'p2', tokenId: 'token-2', avgPrice: '0.40', size: '50.00', realizedPnl: '0' }),
            ];
            ...
            (redis.get as ReturnType<typeof vi.fn>).mockImplementation((key: string) => {
                if (key === 'cache:price:token-1') return Promise.resolve(JSON.stringify({ price: '0.70' }));
                if (key === 'cache:price:token-2') return Promise.resolve(JSON.stringify({ price: '0.60' }));
