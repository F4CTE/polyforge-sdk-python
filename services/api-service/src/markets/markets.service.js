...
let MarketsService = MarketsService_1 = class MarketsService {
    ...
    async list(query) {
        ...
        if (closed !== undefined)
            where.closed = closed;
        const orderBy = sort === "endDate" || sort === "closing_soon"
            ? { endDate: "asc" }
        ...
        if (cachedCount) {
            ...
            if (!hasFilters) {
                const estimate = await this.prisma.$queryRaw `
    ...
    `, ...params, limit, skip);
        const result = (0, pagination_dto_1.paginate)(rows, total, page, limit);
        // Cache for 120 seconds
        await this.redis.set(cacheKey, JSON.stringify(result), 120);
        return result;
    }
    async findOne(marketId) {
    ...
    async priceHistory(tokenId, query) {
