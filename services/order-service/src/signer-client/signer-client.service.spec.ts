...
describe('SignerClientService', () => {
    ...
    describe('signOrder() — success', () => {
        ...
        it('attaches Authorization: Bearer <jwt> header', async () => {
            await svc.signOrder(SIGN_REQ);
            expect(fetchSpy.mock.calls[0][1].headers.Authorization).toBe('Bearer mock-service-jwt');
        });
    ...
    describe('signOrder() — errors', () => {
        it('throws ServiceUnavailableException when fetch rejects (ECONNREFUSED)', async () => {
            ...
            await expect(svc.signOrder(SIGN_REQ)).rejects.toBeInstanceOf(ServiceUnavailableException);
        });
    ...
    describe('URL configuration', () => {
        it('uses the configured SIGNER_SERVICE_URL', async () => {
            const customSvc = new SignerClientService(makeJwt(), makeConfig('http://custom-signer:9999'));
            fetchSpy.mockResolvedValue({ ok: true, json: vi.fn().mockResolvedValue(SIGN_RESPONSE) });

            await customSvc.signOrder(SIGN_REQ);

            expect(fetchSpy.mock.calls[0][0]).toBe('http://custom-signer:9999/sign/order');
        });
