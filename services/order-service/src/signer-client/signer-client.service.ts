...
export class SignerClientService {
    ...
    constructor(
        ...
        this.signerUrl = this.config.get<string>('SIGNER_SERVICE_URL') ?? 'http://signer-service:3012';
    }
    ...
    async signOrder(req: SignOrderRequest): Promise<SignedOrder> {
        ...
        if (!res.ok) {
            ...
            throw new ServiceUnavailableException(`signer-service error ${res.status}: ${body}`);
        }
