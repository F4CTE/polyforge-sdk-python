...
describe('OrdersService', () => {
    ...
    describe('processIntent() — happy path', () => {
        ...
        it('updates to SUBMITTED after signing but before CLOB call', async () => {
            ...
            clob.submitOrder.mockImplementation(async () => {
                const hasSubmitted = prisma.order.update.mock.calls
                    .some(([args]: any[]) => args.data?.status === 'SUBMITTED');
        ...
        it('updates to LIVE when CLOB returns status LIVE', async () => {
            ...
            const liveCall = prisma.order.update.mock.calls
                .find(([args]: any[]) => args.data?.status === 'LIVE');
        ...
        it('updates to MATCHED when CLOB returns status MATCHED', async () => {
            ...
            const matchedCall = prisma.order.update.mock.calls
                .find(([args]: any[]) => args.data?.status === 'MATCHED');
        ...
        it('updates to CONFIRMED when CLOB returns status FILLED', async () => {
            ...
            const confirmedCall = prisma.order.update.mock.calls
                .find(([args]: any[]) => args.data?.status === 'CONFIRMED');
        ...
        it('emits ORDER_PLACED event after successful submission', async () => {
            ...
            expect(events.emitOrderPlaced).toHaveBeenCalledWith('user-1', expect.any(String), 'intent-1');
        });
        ...
        it('passes expiration when provided in the intent', async () => {
            const p = svc.processIntent(makeIntent({ expiration: 1_700_000_000, orderType: 'GTD' }));
            await vi.runAllTimersAsync();
            await p;
        ...
        it('does not write to DLQ on success', async () => {
            ...
            const dlqCalls = redis.xadd.mock.calls
                .filter(([stream]: any[]) => stream === 'stream:orders:dlq');
    ...
    describe('processIntent() — retry / DLQ', () => {
        ...
        it('sends to DLQ after 3 failed attempts', async () => {
            ...
            expect(redis.xadd).toHaveBeenCalledWith(
                'stream:orders:dlq',
                expect.objectContaining({ reason: expect.stringContaining('permanent failure') }),
            );
        });
        ...
        it('emits ORDER_FAILED event after exhausting all retries', async () => {
            ...
            expect(events.emitOrderFailed).toHaveBeenCalledWith('user-1', expect.any(String), expect.any(String));
        });
        ...
        it('updates order status to FAILED after DLQ', async () => {
            ...
            const failedUpdate = prisma.order.update.mock.calls
        ...
        it('DLQ entry contains intent metadata', async () => {
            ...
            const dlqCall = redis.xadd.mock.calls.find(([s]: any[]) => s === 'stream:orders:dlq')!;
    ...
    describe('processBatch()', () => {
        ...
        it('processes exactly 15 intents (one full batch)', async () => {
            const intents = Array.from({ length: 15 }, (_, i) => makeIntent({ intentId: `i-${i}` }));
        ...
        it('processes 20 intents in two chunks (15 + 5)', async () => {
            const intents = Array.from({ length: 20 }, (_, i) => makeIntent({ intentId: `i-${i}` }));
    ...
    describe('mapClobStatus() — via processIntent (indirect)', () => {
        it('maps CANCELLED status correctly', async () => {
            clob.submitOrder.mockResolvedValue({ orderID: 'clob-c', status: 'CANCELLED' });

            const p = svc.processIntent(makeIntent());
            await vi.runAllTimersAsync();
            await p;

            const cancelledUpdate = prisma.order.update.mock.calls
                .find(([args]: any[]) => args.data?.status === 'CANCELLED');
            expect(cancelledUpdate).toBeDefined();
        });

        it('maps unknown CLOB status to SUBMITTED', async () => {
            clob.submitOrder.mockResolvedValue({ orderID: 'clob-u', status: 'UNKNOWN_STATUS' });

            const p = svc.processIntent(makeIntent());
            await vi.runAllTimersAsync();
            await p;

            const submittedUpdate = prisma.order.update.mock.calls
                .find(([args]: any[]) => args.data?.status === 'SUBMITTED');
    ...
    describe('closePosition()', () => {
        ...
        it('uses provided strategyId when given', async () => {
            const p = svc.closePosition('user-1', 'token-1', 'market-1', '5', 'strat-99');
            await vi.runAllTimersAsync();
            await p;
