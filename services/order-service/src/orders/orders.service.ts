...
export class OrdersService {
    ...
    async processIntent(intent: OrderIntent, attempt = 1): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Failed to create order record for intent ${intent.intentId}`, err);
        ...
        try {
            ...
            // Submit to CLOB
            const clobResponse = await this.clob.submitOrder({ order, builderHeaders });
            ...
            await this.events.emitOrderPlaced(intent.userId, orderId, intent.intentId);
            this.logger.log(`Order placed: ${orderId} (CLOB: ${clobResponse.orderID})`);

        } catch (err) {
            const errMsg = err instanceof Error ? err.message : String(err);
            this.logger.warn(`Order attempt ${attempt}/${MAX_ATTEMPTS} failed for intent ${intent.intentId}: ${errMsg}`);
