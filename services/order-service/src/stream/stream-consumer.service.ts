...
export class StreamConsumerService implements OnModuleInit, OnModuleDestroy {
    ...
    private async pollOnce(): Promise<void> {
        ...
        const byUser = new Map<string, Array<{ msgId: string; intent: OrderIntent }>>();
        ...
        await Promise.allSettled([...byUser.entries()].map(async ([, items]) => {
    ...
    private parseIntent(fields: string[]): OrderIntent | null {
        try {
            ...
            return {
                ...
                expiration: obj['expiration'] ? parseInt(obj['expiration'], 10) : undefined,
            };
        } catch {
            return null;
        }
