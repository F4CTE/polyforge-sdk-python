...
function makeMetricsMock(overrides: Partial<ReturnType<MetricsService['compute']>> = {}) {
    return {
...
describe('BacktestService — applyFill', () => {
    ...
    it('BUY creates a new position with correct size and avgPrice', () => {
        ...
        const fill: SimFill = { side: 'BUY', outcome: 'YES', size: 10, price: 0.5, tokenId: 'tok-a', type: 'buy_yes' };
    ...
    it('BUY increments betsToday and totalOrders', () => {
        const state     = createSimState();
        const positions = new Map();
        const fill: SimFill = { side: 'BUY', outcome: 'YES', size: 5, price: 0.6, tokenId: 'tok-a', type: 'buy_yes' };
    ...
    it('second BUY averages position cost correctly', () => {
        ...
        // 20 shares @ 0.50
        svc.applyFill(
            { side: 'BUY', outcome: 'YES', size: 20, price: 0.5, tokenId: 'tok-a', type: 'buy_yes' },
            positions, state, 0, simAt,
        );
        // 10 shares @ 0.80
        svc.applyFill(
            { side: 'BUY', outcome: 'YES', size: 10, price: 0.8, tokenId: 'tok-a', type: 'buy_yes' },
            positions, state, 0, simAt,
        );
    ...
    it('SELL computes realized PnL = (price - avgPrice) * size', () => {
        ...
        const fill: SimFill = { side: 'SELL', outcome: 'YES', size: 10, price: 0.7, tokenId: 'tok-a', type: 'scale_out' };
        const rec = svc.applyFill(fill, positions, state, 0, new Date());
    ...
    it('SELL adds pnl to prevCumPnl to produce equityCurve', () => {
        ...
        const fill: SimFill = { side: 'SELL', outcome: 'YES', size: 10, price: 0.6, tokenId: 'tok-a', type: 'scale_out' };
    ...
    it('SELL removes position when remaining size ≤ 0.0001', () => {
        ...
        svc.applyFill(
            { side: 'SELL', outcome: 'YES', size: 10, price: 0.7, tokenId: 'tok-a', type: 'scale_out' },
            positions, state, 0, new Date(),
        );
    ...
    it('SELL reduces position size when partially sold', () => {
        ...
        svc.applyFill(
            { side: 'SELL', outcome: 'YES', size: 4, price: 0.7, tokenId: 'tok-a', type: 'scale_out' },
            positions, state, 0, new Date(),
        );
    ...
    it('SELL with no existing position produces pnl=0 and unchanged equityCurve', () => {
        ...
        const fill: SimFill = { side: 'SELL', outcome: 'YES', size: 5, price: 0.7, tokenId: 'tok-x', type: 'scale_out' };
    ...
    it('losing SELL increments consecutiveLoss', () => {
        ...
        svc.applyFill(
            { side: 'SELL', outcome: 'YES', size: 10, price: 0.5, tokenId: 'tok-a', type: 'scale_out' },
            positions, state, 0, new Date(),
        );
...
describe('BacktestService — finalize()', () => {
    it('calls metrics.compute with all fill records', async () => {
        const prisma  = makePrismaMock();
        const redis   = makeRedisMock();
        const metrics = makeMetricsMock({ totalPnl: 42, winRate: 0.75, maxDrawdown: 5, sharpeRatio: 1.2 });
    ...
    it('emits BACKTEST_PROGRESS event with progress=100 to stream:events', async () => {
        ...
        expect(redis.xadd).toHaveBeenCalledWith(
            'stream:events',
            expect.objectContaining({ type: 'BACKTEST_PROGRESS', progress: '100', runId: 'run-final' }),
        );
    });
...
describe('BacktestService — run() error handling', () => {
    it('marks run as FAILED when an unexpected error is thrown', async () => {
        ...
        prisma.backtestRun.findUniqueOrThrow.mockRejectedValue(new Error('DB is down'));
