...
export class BacktestService {
    ...
    async run(runId: string): Promise<void> {
        ...
        await this.prisma.backtestRun.update({ where: { id: runId }, data: { status: 'RUNNING' } });
        await this.setProgress(runId, 0);

        try {
            const run = await this.prisma.backtestRun.findUniqueOrThrow({ where: { id: runId } });
            ...
            const ticks = await this.fetchTicks(tokenIds, dateRangeStart, dateRangeEnd);
            ...
            for (let i = 0; i < ticks.length; i++) {
                ...
                for (const fill of autoFills) {
                    const rec = this.applyFill(fill, positions, state, cumulativePnl, tick.time);
                ...
                if (!checkSafety(safety, state, prices, positions)) {
                    this.logger.log(`Safety halt at tick ${i} (${tick.time.toISOString()}) for run ${runId}`);
                    break;
                }

                // Trigger + condition gates
                if (!checkTriggers(triggers, prices)) continue;
                if (!checkConditions(conditions, state, prices, positions, nowMs)) continue;
                ...
                if (pending.length >= BATCH_SIZE) {
                    await this.prisma.backtestOrder.createMany({ data: pending.splice(0, BATCH_SIZE) });
                }
            ...
            this.logger.error(`Backtest run ${runId} failed: ${err?.message}`, err?.stack);
            await this.prisma.backtestRun.update({
                where: { id: runId },
                data: { status: 'FAILED', errorMessage: String(err?.message ?? 'Unknown error') },
            });
        }
    }
    ...
    private async fetchTicks(tokenIds: string[], start: Date, end: Date): Promise<PriceSnapshot[]> {
    ...
    private async finalize(
        ...
        this.logger.log(`Backtest run ${runId} completed — fills: ${fills.length}, pnl: ${computed.totalPnl.toFixed(2)}`);
    }

    private async setProgress(runId: string, progress: number): Promise<void> {
        await this.redis.getClient().set(PROGRESS_KEY(runId), String(progress), 'EX', 3600);
    }

    private async emitProgress(runId: string, userId: string, progress: number): Promise<void> {
