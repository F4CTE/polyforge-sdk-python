...
function makeFill(overrides: Partial<FillRecord> & Pick<FillRecord, 'side' | 'pnl' | 'equityCurve'>): FillRecord {
    return {
...
function makeDailyFills(equities: number[]): FillRecord[] {
    return equities.map((eq, i) => ({
        ...
        equityCurve: eq,
        simulatedAt: new Date(`2024-01-${String(i + 1).padStart(2, '0')}T12:00:00Z`),
    }));
}
...
describe('MetricsService — Sharpe ratio calculation', () => {
    ...
    it('scales by sqrt(252) annualisation factor', () => {
        ...
        const fills: FillRecord[] = [
            { side: 'SELL', pnl: 10, equityCurve: 10, simulatedAt: new Date('2024-01-01T00:00:00Z') },
            { side: 'SELL', pnl: 30, equityCurve: 40, simulatedAt: new Date('2024-01-02T00:00:00Z') },
            { side: 'SELL', pnl: 10, equityCurve: 50, simulatedAt: new Date('2024-01-03T00:00:00Z') },
        ];
