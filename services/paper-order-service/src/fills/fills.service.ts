...
export class FillsService {
    ...
    async simulate(intent: OrderIntent): Promise<void> {
        ...
        if (realizedPnl !== 0) {
            await this.redis.getClient().incrbyfloat(PAPER_PNL_KEY(intent.userId), realizedPnl);
        }
        ...
        this.logger.log(`Paper fill: ${intent.side} ${fillSize} ${intent.tokenId} @ ${fillPrice} for user ${intent.userId}`);
    }
    ...
    private async upsertPosition(intent: OrderIntent, fillPrice: number, fillSize: number): Promise<number> {
        const existing = await this.prisma.paperPosition.findUnique({
            where: { userId_tokenId: { userId: intent.userId, tokenId: intent.tokenId } },
        });
        ...
        if (!existing) {
            ...
            if (intent.side === 'BUY') {
                ...
                const newAvg  = (existingSize * existingAvg + fillSize * fillPrice) / newSize;
