...
class TestTradingCopyNumericValidation:
    ...
    def test_update_copy_config_silently_drops_unknown_kwargs(self):
        from unittest.mock import MagicMock
        ...
        assert "sizeValue" in json_body
        assert json_body["sizeValue"] == "5"
        assert "unknownField" not in json_body
        assert "anotherUnknown" not in json_body
        client.close()
