...
export class SettingsPage {
  ...
  constructor(page: Page) {
    ...
    this.notificationsTab = page.locator('[role="tab"]', {
    ...
    this.saveProfileButton = page.locator("button", {
      hasText: "Save Profile",
    });
    ...
    this.notificationCheckboxes = {
      orderFilled: page
        .locator('[role="switch"][aria-label*="Order Filled"]')
        .first(),
      strategyError: page
        .locator('[role="switch"][aria-label*="Strategy Error"]')
        .first(),
      backtestComplete: page
        .locator('[role="switch"][aria-label*="Backtest Complete"]')
        .first(),
      priceAlert: page
        .locator('[role="switch"][aria-label*="Price Alert"]')
        .first(),
      dailyLossLimit: page
        .locator('[role="switch"][aria-label*="Daily Loss Limit"]')
        .first(),
      marketResolved: page
        .locator('[role="switch"][aria-label*="Market Resolved"]')
        .first(),
      newFollower: page
        .locator('[role="switch"][aria-label*="New Follower"]')
        .first(),
    };
    // Button text is "Save Preferences" (not "Save Notifications")
    this.saveNotificationsButton = page.locator("button", {
      hasText: "Save Preferences",
    });
    ...
    // The actual button text is "Verify & Enable" (not "Enable 2FA")
    this.enable2faButton = page.locator("button", {
      hasText: "Verify & Enable",
    });
    this.disable2faButton = page.locator("button", { hasText: "Disable 2FA" });
    // Backup codes are monospaced spans inside the twofa-panel (no dedicated testid).
    // The twofa-panel backup view renders codes in a <span> with font-mono class.
    this.backupCodes = page.locator(
      '[data-testid="twofa-panel"] span.font-mono',
    );
    ...
    this.createKeyButton = page.locator("button", {
      hasText: "Generate API Key",
    });
    ...
    this.createdKeyDisplay = page.locator(
      "code.text-warning, code.text-pf-warning",
    );
    ...
    this.dailyLimit = page
      .locator('[data-testid="gas-panel"] span', { hasText: "Daily Limit" })
      .first();
    this.remaining = page
      .locator('[data-testid="gas-panel"] span', { hasText: "Remaining" })
      .first();

    // Delete account
    this.deleteAccountButton = page.locator("button", {
      hasText: "Delete Account",
    });
    this.deleteConfirmDialog = page.locator('[role="dialog"]', {
      hasText: "Delete Account",
    });
    this.deletePasswordInput = page.locator(
      '[role="dialog"] input[type="password"]',
    );
    this.deleteConfirmButton = page.locator('[role="dialog"] button', {
      hasText: "Delete",
    });
  }
  ...
  async toggleNotification(
    name: keyof typeof this.notificationCheckboxes,
  ): Promise<void> {
  ...
  async disable2FA(password: string, totpCode: string): Promise<void> {
    ...
    // The disable form appears inline (not a dialog) with password + TOTP inputs.
    // Fill required fields and click "Confirm Disable".
    const disablePassword = this.page.locator("input#2fa-disable-password");
    const disableTotp = this.page.locator("input#2fa-disable-totp-code");
    const confirmBtn = this.page.locator("button", {
      hasText: "Confirm Disable",
    });

    await expect(disablePassword).toBeVisible({ timeout: 5_000 });
    await disablePassword.fill(password);
    await expect(disableTotp).toBeVisible({ timeout: 2_000 });
    await disableTotp.fill(totpCode);
  ...
  async revokeApiKey(keyName: string): Promise<void> {
    ...
    await expect(this.getRevokeButton(keyName))
