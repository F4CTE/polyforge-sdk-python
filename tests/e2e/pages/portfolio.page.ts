...
export class PortfolioPage {
  ...
  constructor(page: Page) {
    ...
    this.resetPaperButton = page.locator("button", {
      hasText: "Reset Paper Account",
    });
    this.resetConfirmDialog = page.locator('[role="dialog"]');
    this.resetConfirmButton = page.locator('[role="dialog"] button', {
      hasText: "Reset",
    });
    this.resetCancelButton = page.locator('[role="dialog"] button', {
      hasText: "Cancel",
    });
  }
  ...
  async goto(): Promise<void> {
    ...
    await expect(this.page.locator("h1", { hasText: "Portfolio" })).toBeVisible(
      { timeout: 15_000 },
    );
  }
  ...
  async switchToLive(): Promise<void> {
    ...
    await expect(this.liveTab).toHaveAttribute("aria-selected", "true", {
      timeout: 10_000,
    });
  }
  ...
  async switchToPaper(): Promise<void> {
    ...
    await expect(this.paperTab).toHaveAttribute("aria-selected", "true", {
      timeout: 10_000,
    });
  }
  ...
  async waitForPaperLoaded(): Promise<void> {
    ...
    const ready = this.page
      .locator('text="Paper P&L"')
      .or(this.page.locator('text="No paper positions"'))
      .first();
    const isPaperRequest = (url: string) =>
      url.includes("/api/v1/paper/summary");
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      const paperRequestSettled = Promise.race([
        this.page
          .waitForResponse((response) => isPaperRequest(response.url()), {
            timeout: 8_000,
          })
      ...
      await this.paperTab.click({ force: true });
      await expect(this.paperTab).toHaveAttribute("aria-selected", "true", {
        timeout: 5_000,
      });
      ...
      if (
        responseOk &&
        (await ready.isVisible({ timeout: 5_000 }).catch(() => false))
      )
        return;
    }
