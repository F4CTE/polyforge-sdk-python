...
test.describe("Strategy lifecycle", () => {
  ...
  test("full lifecycle: create → start paper → pause → resume → stop", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    const builderPage = new StrategyBuilderPage(page);
  ...
  test("edit strategy name via builder", async ({ page }) => {
    ...
    const { id: strategyId } = await apiCreateStrategy(
      freshResp.token,
      original,
    );
    ...
    await expect(builderPage.nameInput).toHaveValue(original, {
      timeout: 10_000,
    });
  ...
  test("builder adds a block to the Triggers section", async ({ page }) => {
    ...
    await expect(builderPage.blockCards().first()).toBeVisible({
      timeout: 5_000,
    });
