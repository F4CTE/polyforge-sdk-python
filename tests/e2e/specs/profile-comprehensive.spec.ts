...
test.describe.serial("Profile — Full Workflow Coverage", () => {
  ...
  test("my profile displays bio if present", async ({ page }) => {
    ...
    // Bio may be empty for new users — verify it's a valid string.
  ...
  test("my profile displays avatar if present", async ({ page }) => {
    ...
    const avatar = page.locator(
      '[data-testid="profile-avatar"], img[alt="avatar"]',
    );
    // Avatar may not be present for all users — just verify page loaded
    await expect(avatar.first())
  ...
  test("@smoke edge rating score is displayed", async ({ page }) => {
    ...
    // Edge rating may be not computed yet — accept empty string or numeric string.
    expect(edgeRating === "" || !isNaN(Number(edgeRating))).toBe(true);
  });
  ...
  test("edge rating shows score breakdown metrics", async ({ page }) => {
    ...
    for (const metric of metrics) {
      ...
      // Verify page is loaded and search for each metric; may be absent
      await expect(metricElement.first())
  ...
  test("edge rating metrics display values", async ({ page }) => {
    ...
    // Verify metric value elements are present if user has trading history
    const metricValues = page.locator('[data-testid="metric-value"]');
    await expect(metricValues.first())
      .toBeVisible({ timeout: 5_000 })
      .catch(() => {});
  });
  ...
  test("edge rating is color-coded based on value", async ({ page }) => {
    ...
    const ratingDisplay = page.locator(
      '[data-testid="edge-rating"], [class*="rating"]',
    );
    const color = await ratingDisplay.evaluate(
      (el) => window.getComputedStyle(el).color,
    );

    // Should be a valid CSS color (rgb/rgba format), not undefined or empty
  ...
  test("@smoke badges grid is displayed", async ({ page }) => {
    ...
    const badgesContainer = page.locator(
      '[data-testid="badges-container"], [data-testid="badges"]',
    );
    await expect(badgesContainer.first())
      .toBeVisible({ timeout: 5_000 })
      .catch(() => {});
  });
  ...
  test("earned badges have date displayed", async ({ page }) => {
    ...
    const earnedBadges = page.locator(
      '[data-testid="badge"][data-earned="true"]',
    );
  ...
  test("unearned badges are dimmed or locked", async ({ page }) => {
    ...
    const unearnedBadges = page.locator(
      '[data-testid="badge"][data-earned="false"]',
    );
  ...
  test("@smoke status indicators are visible", async ({ page }) => {
    ...
    await expect(statusChips.first())
      .toBeVisible({ timeout: 5_000 })
      .catch(() => {});
  });
  ...
  test("email verified status chip shown", async ({ page }) => {
    ...
    const emailStatus = page.locator(
      'text=Email Verified, [data-testid="status-email"], [class*="verified"]',
    );
    // May or may not be visible depending on implementation
    await expect(emailStatus.first())
      .toBeVisible({ timeout: 3_000 })
      .catch(() => {});
  });
  ...
  test("email verified status is green when verified", async ({ page }) => {
    ...
    if (await emailStatus.isVisible()) {
      const color = await emailStatus.evaluate(
        (el) => window.getComputedStyle(el).color,
      );
      // Should be green or similar success color (rgb(0, 128+, 0) range)
      expect(color).toMatch(/^rgba?\(\s*0[,/]\s*\d+/);
    }
  });
  ...
  test("polymarket connected status chip shown", async ({ page }) => {
    ...
    const polymarketStatus = page.locator(
      'text=Polymarket Connected, [data-testid="status-polymarket"]',
    );
    await expect(polymarketStatus.first())
      .toBeVisible({ timeout: 3_000 })
      .catch(() => {});
  });
  ...
  test("2FA enabled status chip shown", async ({ page }) => {
    ...
    const twoFAStatus = page.locator(
      'text=2FA Enabled, [data-testid="status-2fa"]',
    );
    await expect(twoFAStatus.first())
      .toBeVisible({ timeout: 3_000 })
      .catch(() => {});
  });
  ...
  test("settings link navigates to /settings", async ({ page }) => {
    ...
    const settingsLink = page
      .locator("main a", { hasText: "Settings" })
      .first();
  ...
  test("trading account link navigates to /settings/trading-account", async ({
    page,
  }) => {
    await profilePage.gotoProfile("me");
  ...
  test("@smoke navigate to /profile/:username shows public profile", async ({
    page,
  }) => {
  ...
  test("public profile shows bio if present", async ({ page }) => {
    ...
    const bio = await profilePage.getBio();
    // Bio may be empty for public profiles — verify it's a valid string.
    expect(typeof bio).toBe("string");
    expect(bio).not.toBeNull();
  });
  ...
  test("public profile shows edge rating if public", async ({ page }) => {
    ...
    const edgeRating = await profilePage.getEdgeRating();
    // Edge rating may be private or not computed — accept empty or numeric string.
    expect(edgeRating === "" || !isNaN(Number(edgeRating))).toBe(true);
  });
  ...
  test("public profile does not show settings button for other users", async ({
    page,
  }) => {
    const otherUsername = TEST_USERNAME === "alice" ? "bob" : "alice";
    await profilePage.gotoProfile(otherUsername);
    ...
    const editProfileLink = page
      .locator('main a, [role="main"] a, .flex-1 a')
  ...
  test("my profile and public profile (self) show same display name", async ({
    page,
  }) => {
    await profilePage.gotoProfile("me");
  ...
  test("my profile and public profile (self) show same username", async ({
    page,
  }) => {
    await profilePage.gotoProfile("me");
  ...
  test("my profile and public profile (self) show same bio", async ({
    page,
  }) => {
    await profilePage.gotoProfile("me");
  ...
  test("profile data matches after editing profile settings", async ({
    page,
  }) => {
  ...
  test("profile loads and displays all major sections", async ({ page }) => {
    ...
    const profileSection = page.locator(
      '[data-testid="profile-section"], [class*="profile"]',
    );
    const edgeRatingSection = page.locator(
      '[data-testid="edge-rating"], [class*="rating"]',
    );
    const badgesSection = page.locator(
      '[data-testid="badges"], [class*="badges"]',
    );
  ...
  test("can navigate from profile to settings and back to profile", async ({
    page,
  }) => {
    await profilePage.gotoProfile("me");
  ...
  test("can navigate from profile to strategies page", async ({ page }) => {
    ...
    if (
      await myStrategiesLink.isVisible({ timeout: 3_000 }).catch(() => false)
    ) {
  ...
  test("profile page is responsive and loads on different screen sizes", async ({
    page,
  }) => {
  ...
  test("profile page loads without errors", async ({ page }) => {
    ...
    // Should not have critical errors — verify no unhandled JS errors were logged.
    // (May have minor console errors, but shouldn't break the page)
    await expect(
      page.locator(
        '[data-testid="profile-avatar"], img[alt="avatar"], [data-testid="profile-username"], h1',
      ),
    ).toBeVisible({ timeout: 10_000 });
    expect(
      errors.filter((e) => !e.includes("favicon") && !e.includes("third-party"))
        .length,
    ).toBe(0);
  });
});
